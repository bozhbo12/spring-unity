...
public class _CameraShake : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        if (cameras.Count < 1)
        {
            LogSystem.LogWarning("Camera Shake: No cameras assigned in the inspector!");
        }
        instance = this;
    }
    #endregion
    ...
    private IEnumerator DoShake_Internal(Camera cam, Vector3 seed, int numberOfShakes, Vector3 shakeAmount, Vector3 rotationAmount, float distance, float speed, float decay, float guiShakeModifier, bool multiplyByTimeScale, System.Action callback)
    {
        ...
        vTemp.x = shakeRect.x;
        vTemp.y = shakeRect.y;
        ShakeState state = new ShakeState(cachedTransform.position, cachedTransform.rotation, vTemp);
        ...
        while (currentShakes > 0)
        {
            ...
            vTemp2.x = start2.x - (mod1 * Mathf.Sin(timer) * (shakeAmount.x * shakeDistance * pixelScale));
            vTemp2.y = start2.y - (mod2 * Mathf.Cos(timer) * (shakeAmount.y * shakeDistance * pixelScale));
            state.guiShakePosition = vTemp2;
    ...
    {
        if (ResolutionConstrain.Instance.width != shakeRect.width || ResolutionConstrain.Instance.height != shakeRect.height)
        {

            shakeRect.width = ResolutionConstrain.Instance.width;
            shakeRect.height = ResolutionConstrain.Instance.height;
        }
    }
