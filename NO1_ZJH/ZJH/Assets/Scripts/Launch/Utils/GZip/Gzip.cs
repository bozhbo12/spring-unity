...
public class GZip
{
	...
	public static GZipResult Compress(FileInfo[] files, string lpBaseFolder, string lpDestFolder, string zipFileName, bool deleteTempFile)
	{
		...
		{
            Debug.LogError("Compress zip package error ex4 : " + ex4.Message);
			result.Errors = true;
		}
		return result;
	}
	...
	private static void CreateZipFile(string lpSourceFile, string lpZipFile, GZipResult result)
	{
		...
		{
            Debug.LogError("CreateZipFile  error ex1 : " + ex1.Message);
            result.Errors = true;
		}
		finally
		{
	...
	private static void CreateTempFile(FileInfo[] files, string lpBaseFolder, string lpTempFile, GZipResult result)
	{
		...
		if (files != null && files.Length > 0)
		{
			try
			{
				...
				{
					lpFolder = fi.DirectoryName.Replace("\\", "/") + "/";
					try
					{
					...
					{
                        Debug.LogError("CreateTempFile error ex1 : " + ex1.Message);
                        result.Errors = true;
					}
					finally
					{
			...
			{
                Debug.LogError("CreateTempFile  error ex2 : " + ex2.Message);
                result.Errors = true;
			}
			finally
			{
    ...
    public static GZipResult Decompress(string lpDestFolder, byte[] srcBytes)
    {
        return Decompress(lpDestFolder, srcBytes, true, true, null, null, 4096);
    }
	                                    ...
	                                    bool deleteTempFile, bool writeFiles, string addExtension, Hashtable htFiles, int bufferSize)
	{
		...
		if (!string.IsNullOrEmpty(lpSrcFolder) && !lpSrcFolder.EndsWith("/"))
		{
			lpSrcFolder += "/";
		}
		
		if (!string.IsNullOrEmpty(lpDestFolder) && !lpDestFolder.EndsWith("/"))
		{
			lpDestFolder += "/";
		}
			...
			if (fsTemp != null)
			{
				while (fsTemp.Position != fsTemp.Length)
				{
                    ...
                    if (!string.IsNullOrEmpty(line))
					{
						...
						{
							gzfs.Add(gzf);
							lpFilePath = lpDestFolder + gzf.RelativePath.Replace("\\", "/");
		...
		{
            Debug.LogError("Decompress  error ex3 : " + ex3.Message);
            result.Errors = true;
		}
		finally
		{
		...
		{
            Debug.LogError("Decompress error ex4 : " + ex4.Message);
			result.Errors = true;
		}
    ...
    public static GZipResult Decompress(string lpDestFolder, byte[] srcBytes, bool deleteTempFile, bool writeFiles, string addExtension, Hashtable htFiles, int bufferSize)
    {
        GZipResult result = new GZipResult();
        ...
        try
        {
            msTemp = new MemoryStream(srcBytes, 0, srcBytes.Length);
            if (msTemp != null)
            {
                while (msTemp.Position != msTemp.Length)
                {
                    line = null;
                    while (string.IsNullOrEmpty(line) && msTemp.Position != msTemp.Length)
                    {
                        line = ReadLine(msTemp);
                    }
                    ...
                    if (!string.IsNullOrEmpty(line))
                    {
                        ...
                        {
                            gzfs.Add(gzf);
                            lpFilePath = DelegateProxy.StringBuilder(lpDestFolder, gzf.RelativePath.Replace("\\", "/"));
                            lpFolder = GetFolder(lpFilePath);
                            gzf.LocalPath = lpFilePath;
                            ...
                            if (write)
                            {
                                ...
                                // read from fsTemp and write out the file
                                gzf.Restored = WriteFile(msTemp, gzf.Length, lpFilePath + addExtension, bufferSize);
                            }
                            else
                            {
                                // need to advance fsTemp
                                msTemp.Position += gzf.Length;
                            }
                        }
                    }
                }
    ...
    private static string ReadLine(MemoryStream fs)
    {
    ...
    private static bool WriteFile(MemoryStream ms, int fileLength, string lpFile, int bufferSize)
    {
        bool success = false;
        ...
        {
            Debug.LogError("WriteFile error ex2 : " + ex2.Message);
            success = false;
        }
        finally
        {
    ...
    private static string GetFolder(string lpFilePath)
	{
        ...
        if(lpFolder.EndsWith(".unity3d") || lpFolder.EndsWith(".xml"))
        {
            string[] paths = lpFolder.Split('/');
            lpFolder = lpFolder.Replace("/" + paths[paths.Length - 1], "");
        }
	...
	private static FileStream UnzipToTempFile(string lpZipFile, string lpTempFile, GZipResult result)
	{
		...
		{
            Debug.LogError("UnzipToTempFile error ex1 : " + ex1.Message);
            result.Errors = true;
		}
		finally
		{
