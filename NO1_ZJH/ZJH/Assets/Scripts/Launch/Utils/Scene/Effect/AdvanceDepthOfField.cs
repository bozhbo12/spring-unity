...
public class AdvanceDepthOfField : PEBase
{
    ...
    public Shader dofHdrShader = Shader.Find("Snail/Effect/Dof/DepthOfFieldHdr");
    private Material dofHdrMaterial = null;

    public Shader dx11BokehShader = Shader.Find("Snail/Effect/Dof/DX11Dof");
    ...
    public bool blurScreen = false;
    public float blurScreenIntensity = 0.4f;
    ...
    override public Dictionary<string, int> matParams
    {
        get
        {
            ...
            _matParams.Clear();
            _matParams.Add("_pFocalLength", (int)MatParamType.Type_Float);
            _matParams.Add("_pFocalSize", (int)MatParamType.Type_Float);
            _matParams.Add("_pAperture", (int)MatParamType.Type_Float);
            _matParams.Add("_pMaxBlurSize", (int)MatParamType.Type_Float);
            _matParams.Add("_pBlurScreen", (int)MatParamType.Type_Int);
            _matParams.Add("_pBlurScreenIntensity", (int)MatParamType.Type_Float);
            return _matParams;
        }
    }
    ...
    {
        focalLength = varData.GetFloat("_pFocalLength");
        focalSize = varData.GetFloat("_pFocalSize");
        aperture = varData.GetFloat("_pAperture");
        maxBlurSize = varData.GetFloat("_pMaxBlurSize");

        blurScreen = varData.GetInt("_pBlurScreen") > 0 ? true : false;
        blurScreenIntensity = varData.GetFloat("_pBlurScreenIntensity");
    }
    ...
    {
        // 存储太阳光位置
        varData.SetFloat("_pFocalLength", focalLength);
        varData.SetFloat("_pFocalSize", focalSize);
        varData.SetFloat("_pAperture", aperture);
        varData.SetFloat("_pMaxBlurSize", maxBlurSize);

        varData.SetInt("_pBlurScreen", blurScreen == true ? 1 : 0);
        varData.SetFloat("_pBlurScreenIntensity", blurScreenIntensity);
    }
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        else
        {
            ...
            int blurScreenPass = (blurSampleCount == BlurSampleCount.High || blurSampleCount == BlurSampleCount.Medium) ? 23 : 23;
            if (highResolution)
            {
                if (blurScreen == false)
                {
                ...
                {
                    dofHdrMaterial.SetFloat("_BlurScreenIntensity", blurScreenIntensity);
                    dofHdrMaterial.SetVector("_Offsets", new Vector4(0.0f, internalBlurWidth, 0.025f, internalBlurWidth));
                    Graphics.Blit(source, destination, dofHdrMaterial, blurScreenPass);
                }
            }
            else
            {
