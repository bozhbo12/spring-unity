...
//     private Shader shaderMask;
    ...
    private bool CheckMaterialAndShader(Material material, string name)
    {
        ...
        {
            LogSystem.LogWarning("[AmplifyColor] Error creating " + name + " material. Effect disabled.");
            base.enabled = false;
        }
        else if (!material.shader.isSupported)
        {
            LogSystem.LogWarning("[AmplifyColor] " + name + " shader not supported on this platform. Effect disabled.");
            base.enabled = false;
        }
        else
        {
    ...
    {
        return (this.CheckShader(this.shaderBase));
    }
...
//         this.normalLut.Apply();
    ...
    private void OnEnable()
    {
        if (this.CheckSupport())
        {
            ...
            if (((this.LutTexture != null) && (this.LutTexture.mipmapCount > 1)) )
            {
                LogSystem.LogWarning("[AmplifyColor] Please disable \"Generate Mip Maps\" import settings on all LUT textures to avoid visual glitches. Change Texture Type to \"Advanced\" to access Mip settings.");
            }
        }
    }
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        if ((this.JustCopy || !flag))
        {
            Graphics.Blit(source, destination);
        }
        //else if (((this.LutTexture == null) && (this.lutTexture3d == null)) || ((this.BlendAmount == 1f)))
        //{
        //    Graphics.Blit(source, destination);
        //}
        else
...
//         }
    ...
    {
        LogSystem.LogWarning("[AmplifyColor] Error initializing shaders. Please reinstall Amplify Color.");
        base.enabled = false;
    }

    private void ReportNotSupported()
    {
        LogSystem.LogWarning("[AmplifyColor] This image effect is not supported on this platform. Please make sure your Unity license supports Full-Screen Post-Processing Effects which is usually reserved forn Pro licenses.");
        base.enabled = false;
    }
