...
/// </description>
public static class CachePoolManager
{
...
{   
    ...
    public SpawnPool Create(string poolName)
    {
        ...
        SpawnPool oSpawnPool = owner.AddComponent<SpawnPool>();
        if( oSpawnPool != null )
        {
            oSpawnPool.CreateRoot();
        }
        return oSpawnPool;
    }
    ...
    private bool assertValidPoolName(string poolName)
    {
        ...
        {
            if (CachePoolManager.bDebug)
                LogSystem.Log(string.Format("A pool with the name '{0}' already exists", poolName));
    ...
    /// <summary>
    /// DestroyTimer an entire SpawnPool, including its GameObject and all children.
    ...
    public bool Destroy(string poolName)
    {
        ...
        {
            if (CachePoolManager.bDebug)
                LogSystem.LogWarning(string.Format("PoolManager: Unable to destroy '{0}'. Not in PoolManager", poolName));
    ...
    /// <summary>
    /// DestroyTimer ALL SpawnPools, including their GameObjects and all children.
    ...
    internal void Add(SpawnPool spawnPool)
    {
        ...
        {
            if (CachePoolManager.bDebug)
                LogSystem.LogWarning(string.Format("A pool with the name '{0}' already exists. " +
    ...
    /// <summary>
    /// Used internally by SpawnPools to remove themseleves on DestroyTimer().
    ...
    internal bool Remove(SpawnPool spawnPool)
    {
        ...
        {
            if (CachePoolManager.bDebug)
                LogSystem.LogWarning(string.Format("PoolManager: Unable to remove '{0}'. " +
    ...
    public bool Remove(KeyValuePair<string, SpawnPool> item)
    {
        string msg = "SpawnPools can only be destroyed, not removed and kept alive" +
                     " outside of PoolManager. There are only 2 legal ways to destroy " +
                     "a SpawnPool: DestroyTime the GameObject directly, if you have a " +
                     "reference, or use PoolManager.DestroyTime(string poolName).";
        throw new System.NotImplementedException(msg);
    }
