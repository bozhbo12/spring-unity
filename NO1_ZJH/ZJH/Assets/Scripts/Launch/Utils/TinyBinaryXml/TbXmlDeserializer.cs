...
namespace TinyBinaryXml
{
	public class TbXmlDeserializer
	{
		public TbXml DeserializeXmlBytes(byte[] xmlBytes)
		{
            ...
            {
                tbXml.bNewXML = binaryReader.ReadBoolean();
                ...
                DeserializeBoolPool(binaryReader, tbXml);
                DeserializeIntPool(binaryReader, tbXml);
                DeserializeLongPool(binaryReader, tbXml);
                DeserializeFloatPool(binaryReader, tbXml);
                DeserializeDoublePool(binaryReader, tbXml);
        ...
        private void DeserializeBoolPool(BinaryReader binaryReader, TbXml tbXml)
        {
            int numValues = binaryReader.ReadInt32();
            tbXml.boolPool = new List<bool>(numValues);

            for (int i = 0; i < numValues; ++i)
            {
                tbXml.boolPool.Add(binaryReader.ReadBoolean());
            }
        }
        private void DeserializeIntPool(BinaryReader binaryReader, TbXml tbXml)
        {
            int numValues = binaryReader.ReadInt32();
            tbXml.intPool = new List<int>(numValues);

            for (int i = 0; i < numValues; ++i)
            {
                tbXml.intPool.Add(binaryReader.ReadInt32());
            }
        }
        private void DeserializeLongPool(BinaryReader binaryReader, TbXml tbXml)
        {
            int numValues = binaryReader.ReadInt32();
            tbXml.longPool = new List<long>(numValues);

            for (int i = 0; i < numValues; ++i)
            {
                tbXml.longPool.Add(binaryReader.ReadInt64());
            }
        }
        private void DeserializeFloatPool(BinaryReader binaryReader, TbXml tbXml)
        {
            int numValues = binaryReader.ReadInt32();
            tbXml.floatPool = new List<float>(numValues);

            for (int i = 0; i < numValues; ++i)
            {
                tbXml.floatPool.Add(binaryReader.ReadSingle());
            }
        }

        private void DeserializeDoublePool(BinaryReader binaryReader, TbXml tbXml)
        {
            int numValues = binaryReader.ReadInt32();
            tbXml.doublePool = new List<double>(numValues);

            for (int i = 0; i < numValues; ++i)
            {
                tbXml.doublePool.Add(binaryReader.ReadDouble());
            }
        }
		...
		private void DeserializeNode(BinaryReader binaryReader, ushort index, TbXml tbXml)
		{
            ...
            if (numAttributes > 0)
            {
                node.attributeValues = new Dictionary<string, int>(numAttributes);
                for (ushort i = 0; i < numAttributes; ++i)
                {
                    node.attributeValues.Add(binaryReader.ReadString(), binaryReader.ReadInt32());
                }
            }
