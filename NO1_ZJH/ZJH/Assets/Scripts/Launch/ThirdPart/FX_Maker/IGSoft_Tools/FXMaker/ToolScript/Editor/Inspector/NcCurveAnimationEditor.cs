...
public class NcCurveAnimationEditor : FXMakerEditor
{
        ...
        EditorGUI.BeginChangeCheck();
        {
            ...
            for (int n = 0; n < (curveInfoList != null ? curveInfoList.Count : 0); n++)
            {
                ...
                EditorGUI.BeginChangeCheck();
                {
                    ...
                    {
                        GUI.Box(rect, string.Empty);
                        GUILayout.Label(string.Empty, GUILayout.Width(nLeftWidth));
                ...
                else
                {
                    ...
                    if (curveInfoList[n].m_ApplyType != NcCurveAnimation.NcInfoCurve.APPLY_TYPE.RANDOMANGLE)
                    {
                        rect = EditorGUILayout.BeginHorizontal(GUILayout.Height(nLineHeight));
                        {
                            GUI.Box(rect, string.Empty);
                    ...
                    else
                    {
                        ...
                        rect = EditorGUILayout.BeginVertical(GUILayout.Height(nLineHeight));
                        {
                            GUI.Box(rect, string.Empty);
                            rect = EditorGUILayout.BeginHorizontal(GUILayout.Height(nLineHeight));
                            {
                                //GUILayout.Label(string.Empty, GUILayout.Width(nLeftWidth));
                                m_Sel.m_iRoationX_Start = EditorGUILayout.IntField(GetHelpContent("m_iRoationX_Start"), m_Sel.m_iRoationX_Start);
                                m_Sel.m_iRoationX_End = EditorGUILayout.IntField(GetHelpContent("m_iRoationX_End"), m_Sel.m_iRoationX_End);
                            }
                            EditorGUILayout.EndHorizontal();
                            rect = EditorGUILayout.BeginHorizontal(GUILayout.Height(nLineHeight));
                            {
                                m_Sel.m_iRoationY_Start = EditorGUILayout.IntField(GetHelpContent("m_iRoationY_Start"), m_Sel.m_iRoationY_Start);
                                m_Sel.m_iRoationY_End = EditorGUILayout.IntField(GetHelpContent("m_iRoationY_End"), m_Sel.m_iRoationY_End);
                            }
                            EditorGUILayout.EndHorizontal();
                            rect = EditorGUILayout.BeginHorizontal(GUILayout.Height(nLineHeight));
                            {
                                m_Sel.m_iRoationZ_Start = EditorGUILayout.IntField(GetHelpContent("m_iRoationZ_Start"), m_Sel.m_iRoationZ_Start);
                                m_Sel.m_iRoationZ_End = EditorGUILayout.IntField(GetHelpContent("m_iRoationZ_End"), m_Sel.m_iRoationZ_End);
                            }
                            EditorGUILayout.EndHorizontal();
                        }
	...
	protected override void HelpBox(string caption)
	{
		...
		if (caption == string.Empty || caption == "Script")
			str = FXMakerTooltip.GetHsEditor_NcCurveAnimation(string.Empty);
		base.HelpBox(str);
	}
}
