...
public class FxmInfoBackground : NcEffectBehaviour
{
	...
	public void ShowBackground(bool bShow)
	{
		for (int nIndex=0; nIndex < m_ReferenceObjects.Length; nIndex++)
		{
			...
			if (bShow && m_ReferenceObjects[nIndex] != null)
			{
				...
				m_ChildObjects[nChildIndex].name = m_ChildObjects[nChildIndex].name.Replace("(Clone)", string.Empty);
	...
	public void SetCloneObject(int nCloneIndex, GameObject prefab)
	{
		...
		if (m_ChildObjects[nCloneIndex] != null)
		{
			...
			m_CloneThumbFilenames[nCloneIndex]	= string.Empty;
		}
		...
		if (prefab != null)
		{
			...
			m_ChildObjects[nCloneIndex].name = m_ChildObjects[nCloneIndex].name.Replace("(Clone)", string.Empty);
    ...
    void OnDestroy()
    {
        ...
        for (int i = 0; i < m_ReferenceObjects.Length;i++)
        {
            m_ReferenceObjects[i] = null;
        }
    }
    #endregion;
