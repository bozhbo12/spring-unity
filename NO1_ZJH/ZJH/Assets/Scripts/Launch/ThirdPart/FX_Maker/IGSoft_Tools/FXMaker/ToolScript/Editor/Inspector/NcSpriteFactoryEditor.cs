...
// 			m_Sel.m_AtlasMaterial		= (Material)EditorGUILayout.ObjectField(GetHelpContent("m_AtlasMaterial")	, m_Sel.m_AtlasMaterial, typeof(Material), false);
			...
			rect = EditorGUILayout.BeginHorizontal(GUILayout.Height(nAddHeight*2));
			{
				...
				{
#if UNITY_WEBPLAYER
		LogSystem.LogError("In WEB_PLAYER mode, you cannot run the FXMaker.");
		Debug.Break();
#else
					bClickButton	= true;
...
//			showType		= (NcSpriteFactory.SHOW_TYPE)EditorGUILayout.EnumPopup		(GetHelpContent("m_ShowType")	, showType);
			...
			for (int n = 0; n < (spriteList != null ? spriteList.Count : 0); n++)
			{
				...
				if (spriteList[n].m_TextureGUID != string.Empty)
				...
				rect = EditorGUILayout.BeginHorizontal(GUILayout.Height(nLineHeight));
				{
					...
					subRect.width -= nLeftWidth+nLineHeight*2;
 					spriteList[n].m_TextureName = selTexture==null ? string.Empty : selTexture.name;
 					GUI.Label(subRect, (selTexture==null ? string.Empty : "(" + spriteList[n].m_nFrameCount + ") " + selTexture.name));
					GUI.Box(subRect, string.Empty);
				...
				rect = EditorGUILayout.BeginHorizontal(GUILayout.Height(nSpriteHeight));
				{
					...
					EditorGUI.BeginChangeCheck();
					selTexture	= (Texture2D)EditorGUI.ObjectField(subRect, GetHelpContent(string.Empty), selTexture, typeof(Texture2D), false);
					if (EditorGUI.EndChangeCheck())
					{
						if (selTexture != null)
					...
					if (GUI.Button(new Rect(subRect.x+subRect.width-nDelWidth, subRect.y, nDelWidth, subRect.height), GetHelpContent("Rmv")))
						spriteList[n].m_TextureGUID	= string.Empty;
					GUI.Box(rect, string.Empty);
				}
				EditorGUILayout.EndHorizontal();
...
//								m_FxmPopupManager.ShowSelectAudioClipPopup(m_Sel);
	...
	string[] GetSpriteNodeNames()
	{
		...
		for (int n = 0; n < (spriteList != null ? spriteList.Count : 0); n++)
		{
			if (spriteList[n].m_bIncludedAtlas == false || spriteList[n].m_TextureGUID == string.Empty)
				continue;
...
//		bool			bCheckBlack		= false;
		...
		{
			if (m_Sel.m_SpriteList[n].m_bIncludedAtlas == false || m_Sel.m_SpriteList[n].m_TextureGUID == string.Empty)
				continue;
...
// 			Debug.Log(m_Sel.m_SpriteList[n].m_FrameInfos[0].m_FrameUvOffset);
	...
	protected override void HelpBox(string caption)
	{
		...
		if (caption == string.Empty || caption == "Script")
			str = FXMakerTooltip.GetHsEditor_NcSpriteFactory(string.Empty);
		base.HelpBox(str);
	}
}
