...
public class FxmFolderPopup_Texture : FxmFolderPopup
{
	...
	public override bool ShowPopupWindow(Object selObj, bool bSaveDialog)
	{
		...
		// No Shader Selected
		if (currentObj != null && (currentObj.shader == null || currentObj.shader.name == string.Empty))
			currentObj = null;
...
// 			CreateNewShaderMaterials(m_SelectedMaterial);
	...
	void DrawBottomMenuRect(Rect baseRect)
	{
		...
		{
			// image
			if (GUI.Button(imageRect, new GUIContent(string.Empty, m_SelObjectContent.image, m_SelObjectContent.tooltip), GUI.skin.GetStyle("PopupBottom_ImageButton")))
			{
	...
	string GetTextureInfo(Texture tex, bool bShowSubDir)
	{
		...
		if (tex2d != null)
		{
			...
			if (bShowSubDir)
			{
				...
				name = NgFile.PathSeparatorNormalize(name).Replace(m_LoadDirectory, string.Empty);
			} else name = tex.name;
	...
	void SaveMaterial(Material addMat)
	{
		...
		if (m_bUniqueSave == false && m_SelectedTextureMaterials != null)
		{
			foreach (Material mat in m_SelectedTextureMaterials)
			{
				...
				{
					NgUtil.LogMessage(FXMakerTooltip.GetHsToolMessage("MATERIAL_EXISTSAVED", string.Empty));
		...
		while (true)
		{
			string matName = texname + (0<nUniqueCount ? "_"+nUniqueCount.ToString() : string.Empty) + ".mat";
			...
			if (999 < nLoopCount)
			{
                LogSystem.LogWarning("Over Loop ----------------------");
				return;
			}
		}

		AssetDatabase.CreateAsset(addMat, uniquePath);
		NgUtil.LogMessage(FXMakerTooltip.GetHsToolMessage("MATERIAL_NEWSAVED", string.Empty) + "\n" + uniquePath);
