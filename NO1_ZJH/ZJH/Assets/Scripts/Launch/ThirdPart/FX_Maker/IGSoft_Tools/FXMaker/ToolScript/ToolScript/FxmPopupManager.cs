...
public class FxmPopupManager : MonoBehaviour
{
	...
	protected	string							m_ToolMessage			= string.Empty;
	protected	int								m_nHintRectCount		= 0;
	protected	bool							m_bCursorTooltip		= false;
	protected	string							m_ScriptTooltip			= string.Empty;
	protected	string							m_StaticMessage			= string.Empty;

	protected	string							m_ModalMessage			= string.Empty;
	...
	void Awake()
	{
		...
		{
            LogSystem.LogWarning("m_ToolPopupGameObject is null !!!");
		}
	...
	public void OnGUIModalMessage()
	{
		...
		{
			FXMakerMain.inst.ModalMsgWindow(FXMakerLayout.GetWindowId(FXMakerLayout.WINDOWID.MODAL_MSG), FXMakerLayout.GetModalMessageRect(), winModalMessage, string.Empty);
 			GUI.enabled = false;
		}
	}
	...
	{
		if (m_ScriptTooltip != string.Empty)
			ShowScriptTooltip(m_ScriptTooltip);
		if (m_StaticMessage != string.Empty)
...
//		NgGUIDraw.DrawBox(hintRect, NgLayout.m_ColorHelpBox, 1, false);
		...
		GUI.Window(FXMakerLayout.GetWindowId(FXMakerLayout.WINDOWID.HINTRECT)+m_nHintRectCount, hintRect, winEmpty, string.Empty, style);
...
// 		NgUtil.LogMessage("ToolMessage - " + msg);
	...
	public void ShowHierarchyObjectPopup(string scriptFilename, Object selObj)
	{
		if (scriptFilename != null)
		{
			...
			if (m_CurrentHierarchyPopup == null)
				m_CurrentHierarchyPopup	= UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(m_ToolPopupGameObject, "Assets/Scripts/Launch/ThirdPart/FX_Maker/IGSoft_Tools/FXMaker/ToolScript/ToolScript/FxmPopupManager.cs (395,31)", scriptFilename) as FxmPopup;
			if (m_CurrentHierarchyPopup != null)
	...
	bool SetFxmFolderPopup(System.Type fxmFolderPopup)
	{
		...
		if (m_CurrentFolderPopup == null)
            LogSystem.LogWarning("m_CurrentFolderPopup is null");
