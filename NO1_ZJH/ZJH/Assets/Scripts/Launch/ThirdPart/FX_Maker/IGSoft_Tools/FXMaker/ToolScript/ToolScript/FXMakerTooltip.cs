...
public class FXMakerTooltip
{
	...
	{
		return new GUIContent(caption, (tooltip == string.Empty ? Tooltip(caption) : Tooltip(tooltip)));
	}
...
// 		if (param.Length < 4)
// 		{
// 			LogSystem.LogError("GetHintRect(string hintRectStr) - format error");
// 			return new Rect(0, 0, 0, 0);
// 		}
// 		return new Rect(NgUtil.ToFloat(param[0], 0), NgUtil.ToFloat(param[1], 0), NgUtil.ToFloat(param[2], 0), NgUtil.ToFloat(param[3], 0));
	...
	static void TooltipError(string caption)
	{
		if (caption == "_ehcck_")
			return;
        LogSystem.LogWarning("Toolip not found !!! - " + caption);
	}
...
//		return (int)FXMakerOption.inst.m_LanguageType;
	...
	{
		GetHsToolMessage("_ehcck_", string.Empty);
		GetHsScriptMessage("_ehcck_");
		WindowDescription(new Rect(), 0, null);
		GetHcToolMain("_ehcck_", string.Empty);
		GetHsToolBackground("_ehcck_", string.Empty);
		GetHcToolBackground("_ehcck_", string.Empty);
		GetHsToolEffect("_ehcck_", string.Empty);
		GetHcToolEffect("_ehcck_");
		GetHcToolEffect_Camera();
		GetHcEffectControls("_ehcck_", string.Empty);
		...
		GetHcPopup_GameObject("_ehcck_", string.Empty);
	...
	public static GUIContent GetHcToolMain(string caption)
	{
		...
		return GetHcToolMain(caption, string.Empty);
	}
...
// 		CheckValid(HcEffectGizmoStrrings);
	...
	public static GUIContent GetHcEffectHierarchy(string caption)
	{
		...
		switch (caption)
		{
			...
			case "Enable"				:	return GetGUIContent(string.Empty, "- Enable / Disable\n" + HcEffectHierarchy[GetLang(), 15]);
