...
// 			m_TestSphereObject.active = false;
	...
	public void LoadProject()
	{
		...
		if (m_bGroupFoler)
		{
			...
			m_GroupFolerContents[0]	= new GUIContent(string.Empty, string.Empty);
			m_nGroupCount			= 1;
		}
	...
	void SelectToolbar(int nGroupIndex, int nBackgroundIndex)
	{
		...
		{
			LoadBackgroundFolder(string.Empty);
...
// 			Debug.Log(loaddir);
			...
			for (int n=m_BackgroundPrefabs.Length; n < FXMakerOption.inst.m_nMinBottomToolbarCount; n++)
			{
				GUIContent econ = new GUIContent();
				econ.image		= null;
				econ.text		= string.Empty;
				econ.tooltip	= FXMakerTooltip.GetHsToolBackground("EMPTYBACKGROUND_HOVER", string.Empty);
				m_BackgroundContents[n] = econ;
			}
	...
	void winMenuToolbar(int id)
	{
		...
		if (m_bGroupFoler)
		{
			...
			if (GUI.changed)
				SelectToolbar(nGroupIndex, string.Empty);
...
// 			if (newName != string.Empty && m_GroupFolerStrings[m_nGroupIndex] != newName)
// 			{
...
// 			}
	...
	void NewPrefab()
	{
		if (m_bGroupFoler == false ||  m_GroupFolerContents[m_nGroupIndex].text != string.Empty)
		{
			SaveProject();
			if (m_CurrentBackgroundInfo != null)
				DestroyImmediate(m_CurrentBackgroundInfo.gameObject);
...
//				SaveBackgroundPrefab();
			...
			// Thumb Selected
			if (bCloneType && m_CurrentBackgroundInfo.GetClildThumbFilename(nWinIndex) != string.Empty && GUI.Button(new Rect(FXMakerLayout.m_rectInnerMargin.x, 48+FXMakerLayout.m_fScrollButtonHeight+3, FXMakerLayout.GetFixedWindowWidth()-FXMakerLayout.m_rectInnerMargin.x*2, 25), FXMakerTooltip.GetHcToolBackground("Create Thumb", settingObj.name)))
			{
...
//					SaveBackgroundPrefab();
		...
		if (bCloneType == false)
		{
			...
			if (GUI.Button(FXMakerLayout.GetInnerHorizontalRect(subRect, 2, 0, 1), FXMakerTooltip.GetHcToolBackground("Select", string.Empty)))
			{
				FxmPopupManager.inst.ShowSelectPrefabPopup(m_CurrentBackgroundInfo, false, nBackIndex);
//				SaveBackgroundPrefab();
			}
			if (NgLayout.GUIButton(FXMakerLayout.GetInnerHorizontalRect(subRect, 2, 1, 1), FXMakerTooltip.GetHcToolBackground("Open", string.Empty), m_CurrentBackgroundInfo.GetReferenceObject(nBackIndex) != null))
			{
