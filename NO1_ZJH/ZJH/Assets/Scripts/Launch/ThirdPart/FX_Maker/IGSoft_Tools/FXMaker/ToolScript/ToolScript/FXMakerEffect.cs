...
//	protected		string[]			m_EffectNameStrings;	// = new string[gNcLayout.m_nMaxEffectListCount];
	...
	protected		string				m_EditingName			= string.Empty;
...
// 		Debug.Log(defaultEffectPath);
		...
		// defaultEffectPath
		if (defaultEffectPath != null && defaultEffectPath != string.Empty)
		{
			string projectPath	= defaultEffectPath.Replace(FXMakerMain.inst.GetResourceDir(FXMakerMain.TOOLDIR_TYPE.PROJECTS), string.Empty);
	...
	public bool SaveProject()
	{
		...
		{
			NgUtil.LogMessage(FXMakerTooltip.GetHsToolMessage("READONLY_NOTSAVE", string.Empty));
		} else {
			if (0 <= m_nEffectIndex && m_nEffectIndex < m_nEffectCount)
			{
	...
	{
		if (LoadGroup(nProjectIndex, nGroupIndex, string.Empty))
	...
	bool LoadGroup(int nProjectIndex, int nGroupIndex, string groupName)
	{
		...
		if (0 <= m_nProjectIndex && m_nProjectIndex < m_nProjectCount)
		{
			...
			if (m_GroupFolderContents != null)
				...
				{
					if (groupName != string.Empty && m_GroupFolderContents[n].text == groupName)
						nGroupIndex = n;
	...
	void SetActiveEffect(int nEffectIndex)
	{
		...
		{
			if (SaveProject())
				NgUtil.LogMessage(FXMakerTooltip.GetHsToolMessage("TOOL_AUTOSAVE", string.Empty));
		}

		// clear current
		FXMakerMain.inst.ClearCurrentEffectObject(m_CurrentEffectRoot, true);

		m_EditingName	= string.Empty;
...
//			m_EffectNameStrings	= new string[m_nEffectCount];
			...
			// select prefab
			if (selectPrefabPath != string.Empty)
	...
	void BuildContents()
	{
		...
		for (int n=0; n < m_nEffectCount; n++)
		{
			if (m_EffectContents[n] == null)
			{
				...
				subDir = NgFile.PathSeparatorNormalize(subDir).Replace(m_LoadDirectory, string.Empty);
...
//		FXMakerMain.inst.ToggleGlobalLangSkin(false);
		...
		if (bChanged)
		{
			...
			SelectToolbar(nProjectIndex, (m_nProjectIndex != nProjectIndex ? GetGroupIndex(nProjectIndex) : nGroupIndex), string.Empty);
		}
...
// 		SaveProject();
	...
	{
		string	savePath = EditorUtility.SaveFilePanel("ExportPackage", string.Empty, (bAllExport ? m_GroupFolderContents[GetGroupIndex()].text : FXMakerMain.inst.GetOriginalEffectPrefab().name), "unitypackage");
