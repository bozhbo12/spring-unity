...
//		return FXMakertip.GetGUIContentNoTooltip(text);
	...
	{
#if UNITY_WEBPLAYER
		LogSystem.LogWarning("In WEB_PLAYER mode, you cannot run the FXMaker.");
		Debug.Break();
		return null;
#else
		...
		if (m_bCreatePrefab)
		{
			...
			{
                LogSystem.LogWarning("FXMakerMain.inst.m_FxmSpritePrefab : curveCom Count Error!!!");
			}
	...
	string UniquePrefabPath(string basePath)
	{
		...
		while (true)
		{
			string matName = texname + (0<nUniqueCount ? "_"+nUniqueCount.ToString() : string.Empty) + ".prefab";
	...
	string CreateMaterial(string pathTexture)
	{
		...
		switch (m_ShaderType)
		{
			...
			default : 
				{
                    LogSystem.LogWarning("Missing m_ShaderType");
					return string.Empty;
				}
		}
