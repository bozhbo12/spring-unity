...
public class FxmFolderPopup_NcInfoCurve : FxmFolderPopup
{
	...
	public bool ShowPopupWindow(NcCurveAnimation ncCurveAni, int nIndex, bool bSaveDialog)
	{
		...
		{
            LogSystem.LogWarning("ncCurveAni is null");
			return false;
		}
...
// 			GUI.enabled = false;
	...
	protected override void DrawBottomRect(Rect baseRect)
	{
		...
		else guiCon = new GUIContent(string.Empty, m_SelObjectContent.image, m_SelObjectContent.tooltip);
...
// 		GUI.Label(NgLayout.GetInnerVerticalRect(rightRect, 12, 5, 8), (m_SelObjectContent == null ? "[Not Selected]" : m_SelObjectContent.text));
		...
		if (m_bSaveDialog)
		{
			...
			// save
			if (FXMakerLayout.GUIButton(FXMakerLayout.GetInnerHorizontalRect(buttonRect, 2, 1, 1), (bReadOnyFolder ? FXMakerTooltip.GetGUIContent("Save", FXMakerTooltip.GetHsToolMessage("READONLY_FOLDER", string.Empty)) : GetHelpContent("Save")), bSaveEnable))
			{
				SaveAddCurvePrefab(false);
				ClosePopup(true);
			}
			// overwrite
			if (FXMakerLayout.GUIButton(FXMakerLayout.GetInnerHorizontalRect(buttonRect2, 2, 0, 2), (bReadOnyFolder ? FXMakerTooltip.GetGUIContent("Overwrite", FXMakerTooltip.GetHsToolMessage("READONLY_FOLDER", string.Empty)) : GetHelpContent("Overwrite")), (bSaveEnable && 0 <= m_nObjectIndex)))
			{
...
// 		Debug.Log(m_nProjectCount);
		...
		if (0 < m_nProjectCount)
		{
			for (int n=0; n < m_nProjectCount; n++)
				folderStrings[n] = m_CurveAniObjects[n].name.Replace("NcInfoCurve_", string.Empty);
		}
	...
	protected override void LoadObjects()
	{
		...
		for (int n=0; n < m_nObjectCount; n++)
		{
			if (m_ObjectContents[n] == null)
			{
				...
				m_ObjectContents[n].text	= string.Empty;
