...
public class FXMakerMain : MonoBehaviour
{
    ...
    protected string m_EmptyTooltip = string.Empty;
    protected string m_PriorityTooltip = string.Empty;
    ...
    public string GetResourceDir(TOOLDIR_TYPE tooldirType)
    {
        ...
        switch (tooldirType)
        {
            ...
            // 			default: LogSystem.LogError("GetResourceDir(TOOLDIR_TYPE tooldirType) - not define !!!");
        }
    ...
    void Start()
    {
        ...
        if (FXMakerOption.inst.m_bAutoSaveAppPause == false || FXMakerOption.inst.m_bAutoSaveAppQuit == false)
            FxmPopupManager.inst.SetDelayBottomMessage(FXMakerTooltip.GetHsToolMessage("WARRING_AUTOSAVE", string.Empty));
    }
    ...
    {
        LogSystem.Log("OnApplicationPause --------------------------------");
    ...
    void OnApplicationQuit()
    {
        ...
        LogSystem.Log("OnApplicationQuit --------------------------------");
    ...
    void winMenuChange(int id)
    {
        ...
        if ((FXMakerLayout.m_bMinimizeAll || FXMakerLayout.m_bMinimizeTopMenu) == false)
        {
            ...
            if (GUI.Button(FXMakerLayout.GetChildVerticalRect(FXMakerLayout.GetMenuChangeRect(), 0, 3, 1, 1), FXMakerTooltip.GetHcToolMain("FullCapture", FXMakerCapture.GetCaptureScreenShotDir())))
            {
                ...
                {
                    LogSystem.Log(FXMakerCapture.GetCaptureScreenShotDir());
    ...
    void ProcessTooltip()
    {
        ...
        {
            if (m_PriorityTooltip != string.Empty)
            ...
            if (m_Tooltip != string.Empty)
            {
                ...
                if (m_nOnGUICallCount == 2)
                {
                    ...
                    m_PriorityTooltip = string.Empty;
                }
            }
            else
            {
                ...
                if (fxMakerEffect != null && 0 < GetComponent<FXMakerEffect>().IsReadOnlyFolder())
                    msg += "\n" + FXMakerTooltip.GetHsToolMessage("FOLDER_READONLY_BOTTOM", string.Empty);
    ...
    void OnDestroy()
    {
        ...
        m_GuiMainEngSkin = null;
        m_GuiMainGloSkin = null;
        m_GuiMsgSkin = null;
        m_GuiPopupSkin = null;
        m_GuiTooltipSkin = null;
        m_OldGuiSkin = null;

        // --------------------------------
        m_LineBackMaterial = null;
        m_LineColorMaterial = null;
        m_FXMakerSpritePrefab = null;

        // --------------------------------
        m_GrayscaleCamera = null;
        m_FXMakerMouse = null;
        m_FXMakerControls = null;
        m_FXMakerHierarchy = null;
