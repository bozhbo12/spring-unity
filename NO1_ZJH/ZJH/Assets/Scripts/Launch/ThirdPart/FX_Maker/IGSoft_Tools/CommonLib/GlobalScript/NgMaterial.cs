...
public class NgMaterial
{
	...
	public static string SaveMaterial(Material newMat, string savePath, string saveMatName)
	{
		...
		while (true)
		{
			string matName = saveMatName + (0<nUniqueCount ? "_"+nUniqueCount.ToString() : string.Empty) + ".mat";
...
//		NgUtil.LogMessage(FXMakertip.GetHsToolMessage("MATERIAL_NEWSAVED", string.Empty) + "\n" + uniquePath);
