...
public class FxmPopup_GameObject : FxmPopup
{
	...
	void winPopup(int id)
	{
		...
		GUI.Box(FXMakerLayout.GetOffsetRect(topRect, 0, -2, 0, 2), string.Empty);
...
// 		FXMakerMain.inst.ToggleGlobalLangSkin(false);
		if (m_SelectedTransform.name != newName)
		{
			...
			if (newName.Trim() != string.Empty && m_SelectedTransform.gameObject == FXMakerMain.inst.GetOriginalEffectObject())
		...
		{
			if (FXMakerLayout.GUIButton(FXMakerLayout.GetInnerVerticalRect(leftRect, 20, 17, 3), GetHelpContent("Add Component "), (m_ScriptScrings[m_nPopupListIndex] != string.Empty)))
			{
				AddScript(m_ScriptScrings[m_nPopupListIndex]);
...
// 					continue;
	...
	{
		Component com = UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(m_SelectedTransform.gameObject, "Assets/Scripts/Launch/ThirdPart/FX_Maker/IGSoft_Tools/FXMaker/ToolScript/ToolScript/FxmPopup_GameObject.cs (228,19)", scriptName);
		if (com != null)
		{
	...
	{
		return FXMakerTooltip.GetHcPopup_GameObject(text, string.Empty);
	}
