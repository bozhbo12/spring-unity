...
[CustomEditor(typeof(NcSpriteAnimationShare))]

public class NcSpriteAnimationShareEditor : FXMakerEditor
{
    // Attribute ------------------------------------------------------------------------
    protected NcSpriteAnimationShare m_Sel;
    ...
    void OnEnable()
    {
        m_Sel = target as NcSpriteAnimationShare;
        m_UndoManager = new FXMakerUndoManager(m_Sel, "NcSpriteAnimationShare");
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        // 			FxmInfoIndexing.FindInstanceIndexing(m_Sel.transform, false).GetComponent<NcSpriteAnimationShare>().PauseAnimation();
        // 		if (GUILayout.Button("Resume"))
        // 			FxmInfoIndexing.FindInstanceIndexing(m_Sel.transform, false).GetComponent<NcSpriteAnimationShare>().ResumeAnimation();
        ...
        EditorGUI.BeginChangeCheck();
        {
            ...
            m_Sel.m_TextureType = (NcSpriteAnimationShare.TEXTURE_TYPE)EditorGUILayout.EnumPopup(GetHelpContent("m_TextureType"), m_Sel.m_TextureType);

            EditorGUILayout.Space();

            if (m_Sel.m_TextureType == NcSpriteAnimationShare.TEXTURE_TYPE.TileTexture)
            {
                if (GUILayout.Button(GetHelpContent("ConvertTo : TrimTexture")))
                {
                    ...
                    if (m_Sel.m_NcSpriteFrameInfos != null)
                        m_Sel.m_TextureType = NcSpriteAnimationShare.TEXTURE_TYPE.TrimTexture;
                }
            ...
            if (m_Sel.m_TextureType == NcSpriteAnimationShare.TEXTURE_TYPE.TrimTexture)
            {
                if (GUILayout.Button(GetHelpContent("ExportTo : SplitTexture")))
                {
                    string path = FXMakerCapture.GetExportSlitDir();
            ...
            if (m_Sel.m_TextureType == NcSpriteAnimationShare.TEXTURE_TYPE.SpriteFactory)
            {
            }
            EditorGUILayout.Space();
            ...
            m_Sel.m_PlayMode = (NcSpriteAnimationShare.PLAYMODE)EditorGUILayout.EnumPopup(GetHelpContent("m_PlayMode"), m_Sel.m_PlayMode, GUILayout.MaxWidth(Screen.width));
            if (m_Sel.m_PlayMode != NcSpriteAnimationShare.PLAYMODE.SELECT)
                m_Sel.m_fDelayTime = EditorGUILayout.FloatField(GetHelpContent("m_fDelayTime"), m_Sel.m_fDelayTime);

            if (m_Sel.m_PlayMode != NcSpriteAnimationShare.PLAYMODE.RANDOM && m_Sel.m_PlayMode != NcSpriteAnimationShare.PLAYMODE.SELECT)
            {
            ...
            rect = EditorGUILayout.BeginHorizontal(GUILayout.Height(150));
            {
                ...
                if (m_Sel.GetComponent<Renderer>() != null && m_Sel.GetComponent<Renderer>().sharedMaterial != null && m_Sel.GetComponent<Renderer>().sharedMaterial.mainTexture != null)
                {
                    ...
                    if (m_Sel.m_TextureType == NcSpriteAnimationShare.TEXTURE_TYPE.TileTexture)
                    {
                        if (DrawTileTexture(subRect, (m_Sel.m_PlayMode == NcSpriteAnimationShare.PLAYMODE.SELECT), m_Sel.GetComponent<Renderer>().sharedMaterial, m_Sel.m_nTilingX, m_Sel.m_nTilingY, m_Sel.m_nStartFrame, m_Sel.m_nFrameCount, m_Sel.m_nSelectFrame, out nClickFrameIndex))
                        {
                            bClickButton = true;
                            if (bClickButton && m_Sel.m_PlayMode == NcSpriteAnimationShare.PLAYMODE.SELECT)
                    ...
                    if (m_Sel.m_TextureType != NcSpriteAnimationShare.TEXTURE_TYPE.TileTexture)
                    {
                        if (DrawTrimTexture(subRect, (m_Sel.m_PlayMode == NcSpriteAnimationShare.PLAYMODE.SELECT), m_Sel.GetComponent<Renderer>().sharedMaterial, m_Sel.m_NcSpriteFrameInfos, m_Sel.m_nStartFrame, m_Sel.m_nFrameCount, m_Sel.m_nSelectFrame, out nClickFrameIndex))
                        {
                            bClickButton = true;
                            if (bClickButton && m_Sel.m_PlayMode == NcSpriteAnimationShare.PLAYMODE.SELECT)
                                m_Sel.m_nSelectFrame = nClickFrameIndex;
            ...
            if (m_Sel.m_curveAlphaWeight != null)
            {
                ...
                // 				m_Sel.m_fRedAlphaWeight		= EditorGUILayout.Slider(string.Empty, m_Sel.m_fRedAlphaWeight	, 0, 1.0f);
                // 				m_Sel.m_fGreenAlphaWeight	= EditorGUILayout.Slider(string.Empty, m_Sel.m_fGreenAlphaWeight	, 0, 1.0f);
                //	 			m_Sel.m_fBlueAlphaWeight	= EditorGUILayout.Slider(string.Empty, m_Sel.m_fBlueAlphaWeight	, 0, 1.0f);
            }
    ...
    protected override void HelpBox(string caption)
    {
        ...
        if (caption == string.Empty || caption == "Script")
            str = FXMakerTooltip.GetHsEditor_NcSpriteAnimation(string.Empty);
        base.HelpBox(str);
    }
}
