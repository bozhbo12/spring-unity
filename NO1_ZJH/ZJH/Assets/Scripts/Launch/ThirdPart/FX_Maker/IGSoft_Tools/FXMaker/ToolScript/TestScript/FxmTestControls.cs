...
//   			Debug.Log("bNcAni " + bNcAni + ", bParticle " + bParticle + ", bRen " + bRen);
	...
	public static GUIContent[] GetHcEffectControls_Play(float fAutoRet, float timeScale, float timeOneShot1, float timeRepeat1, float timeRepeat2, float timeRepeat3, float timeRepeat4, float timeRepeat5)
	{
		...
		cons[0]	= new GUIContent("AutoRet"	, string.Empty);
		cons[1]	= new GUIContent(timeScale.ToString("0.00")+"x S"	, string.Empty);
		cons[2]	= new GUIContent(timeOneShot1.ToString("0.0")+"x S"	, string.Empty);
		cons[3]	= new GUIContent(timeRepeat1.ToString("0.0")+"s R"	, string.Empty);
		cons[4]	= new GUIContent(timeRepeat2.ToString("0.0")+"s R"	, string.Empty);
		cons[5]	= new GUIContent(timeRepeat3.ToString("0.0")+"s R"	, string.Empty);
		cons[6]	= new GUIContent(timeRepeat4.ToString("0.0")+"s R"	, string.Empty);
		cons[7]	= new GUIContent(timeRepeat5.ToString("0.0")+"s R"	, string.Empty);
	...
	public static GUIContent[] GetHcEffectControls_Trans(AXIS nTransAxis)
	{
		...
		cons[0]	= new GUIContent("Stop"		,  string.Empty);
		cons[1]	= new GUIContent(nTransAxis.ToString() + " Move"	, string.Empty);
		cons[2]	= new GUIContent(nTransAxis.ToString() + " Scale", string.Empty);
		cons[3]	= new GUIContent("Arc"		, string.Empty);
		cons[4]	= new GUIContent("Fall"		, string.Empty);
		cons[5]	= new GUIContent("Raise"	, string.Empty);
		cons[6]	= new GUIContent("Circle"	, string.Empty);
		cons[7]	= new GUIContent("Tornado"	, string.Empty);
		return cons;
	}
	...
	public static GUIContent[] GetHcEffectControls_Rotate()
	{
		...
		cons[0]	= new GUIContent("Rot"	, string.Empty);
		cons[1]	= new GUIContent("Fix"	, string.Empty);
		return cons;
	}
	...
	void winActionToolbar(int id)
	{
		...
		if (FxmTestMain.inst.IsCurrentEffectObject())
		{
			...
			baseRect		= FXMakerLayout.GetChildVerticalRect(popupRect, 0, nRowCount, 0, 1);
			GUI.Box(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount, 1, 1), new GUIContent("Play", string.Empty));
			...
			GUI.Box(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount, 1, 1), new GUIContent("Trans", string.Empty));
		...
		content  = new GUIContent("DistPerTime", string.Empty);
		...
		// TransSpeed Trans ----------------------------------------------
// 		if (GUI.Button(NgLayout.GetInnerHorizontalRect(baseRect, nColCount*2, 23, 1), new GUIContent("1", string.Empty)))
// 			TransSpeed = 1;
		if (GUI.Button(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount*2, 14, 1), new GUIContent("<", string.Empty)))
			TransSpeed = (int)(TransSpeed-1);
		if (GUI.Button(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount*2, 15, 1), new GUIContent(">", string.Empty)))
			TransSpeed = (int)(TransSpeed+1);
		...
		content  = new GUIContent("TimeScale", string.Empty);
		...
		if (timeScale == 0)
		{
			if (GUI.Button(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount, 7, 1), new GUIContent("Resume", string.Empty)))
				timeScale = m_fOldTimeScale;
		} else {
			if (GUI.Button(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount, 7, 1), new GUIContent("Pause", string.Empty)))
				timeScale = 0;
		}
		if (GUI.Button(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount, 8, 1), new GUIContent("Reset", string.Empty)))
			timeScale = 1;
		...
		if (FxmTestMain.inst.IsCurrentEffectObject())
		{
			...
			content  = new GUIContent("ElapsedTime", string.Empty);
			content.text += " " + (Time.time - m_fPlayStartTime).ToString("0.000");
			GUI.Box(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount, 0, 2), content);
			rect = FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount, 2, 5);
			rect.y += 5;
			GUI.HorizontalSlider(rect, Time.time - m_fPlayStartTime, 0.0f, fMaxTime);
			if (GUI.Button(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount*2, 14, 1), new GUIContent("+.5", string.Empty)))
			{
				SetTimeScale(1.0f);
				Invoke("invokeStopTimer", 0.5f);
			}
			if (GUI.Button(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount*2, 15, 1), new GUIContent("+.1", string.Empty)))
			{
				SetTimeScale(0.4f);
				Invoke("invokeStopTimer", 0.1f);
			}
			if (GUI.Button(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount*2, 16, 1), new GUIContent("+.05", string.Empty)))
			{
				SetTimeScale(0.2f);
				Invoke("invokeStopTimer", 0.05f);
			}
			if (GUI.Button(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount*2, 17, 1), new GUIContent("+.01", string.Empty)))
			{
				SetTimeScale(0.04f);
			...
			if (GUI.Button(FXMakerLayout.GetInnerHorizontalRect(baseRect, nColCount, 9, 1), new GUIContent("Restart", string.Empty)))
				CreateInstanceEffect();
		}
	}
