...
//  		Debug.Log("DestroyEffectObject - " + m_CurrentEffect);
    ...
    #region 释放
    //这个真的要加吗？多此一举把,DestroyEffectObject已经有了
...
// 		Debug.Log("OnChangingSprite() - nOldNodeIndex = " + nOldNodeIndex + ", nNewNodeIndex = " + nNewNodeIndex);
    ...
    public void OnAnimationStartFrame(NcSpriteAnimationShare spriteCom)
    {
    }
...
// 		Debug.Log("OnAnimationChangingFrame() - nOldIndex = " + nOldIndex + ", nNewIndex = " + nNewIndex + ", nLoopCount = " + nLoopCount + ", m_nCurrentIndex = " + m_nCurrentIndex);
    ...
    public void OnAnimationChangingFrame(NcSpriteAnimationShare spriteCom, int nOldIndex, int nNewIndex, int nLoopCount)
    {
        // 		Debug.Log("OnAnimationChangingFrame() - nOldIndex = " + nOldIndex + ", nNewIndex = " + nNewIndex + ", nLoopCount = " + nLoopCount + ", m_nCurrentIndex = " + m_nCurrentIndex);
        if (m_SpriteList.Count <= m_nCurrentIndex)
            return;
...
// 		DestroyEffectObject();
    ...
    // 마지막 frame시간 지나면..(다음 loop 첫프레임이 되면), ret가 참이면 애니변경됨
    public bool OnAnimationLastFrame(NcSpriteAnimationShare spriteCom, int nLoopCount)
    {
        if (m_SpriteList.Count <= m_nCurrentIndex)
            return false;
        ...
        {
            NcSpriteAnimationShare ncTarSprite = SetSprite(m_SpriteList[m_nCurrentIndex].m_nNextSpriteIndex) as NcSpriteAnimationShare;
            if (ncTarSprite != null)
            {
                ncTarSprite.ResetAnimation();
                return true;
