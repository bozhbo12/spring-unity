...
public static class NGUISnap
{
    ...
    static void Recalculate(Transform t)
    {
        ...
        if (Vector3.Dot(t.localRotation * Vector3.up, Vector3.up) < 0.999f)
            return;
    ...
    {
        // If the transform is rotated, ignore it
        if (Vector3.Dot(child.localRotation * Vector3.forward, Vector3.forward) < 0.999f)
            return;
    ...
    static public Vector3 Snap(Vector3 pos, Vector3[] local, bool snapToEdges)
    {
        if (snapToEdges && allow)
        {
            ...
            vTemp2.x = float.MaxValue;
            vTemp2.y = float.MaxValue;
            Vector2 best = vTemp2;
