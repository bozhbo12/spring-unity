...
public class UIStretch : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        UICamera.mOnScreenReSize.Add(ScreenSizeChanged);
       // UICamera.onScreenResize += ScreenSizeChanged;
    }

    void OnDestroy()
    {
        UICamera.mOnScreenReSize.Remove(ScreenSizeChanged);
        //add by chenfei
         mTrans=null;
         mWidget=null;
         mSprite=null;
        ...
        if (style != Style.None)
        {
            ...
            else if (pc != null)
            {
                if (pc.clipping == UIDrawCall.Clipping.None)
                {
                    ...
                    float ratio = (mRoot != null) ? (float)mRoot.activeHeight / ResolutionConstrain.Instance.height * 0.5f : 0.5f;
                    mRect.xMin = -ResolutionConstrain.Instance.width * ratio;
                    mRect.yMin = -ResolutionConstrain.Instance.height * ratio;
            ...
            vTemp.x = mWidget.width;
            vTemp.y = mWidget.height;

            Vector3 size = (mWidget != null) ? vTemp : mTrans.localScale;
