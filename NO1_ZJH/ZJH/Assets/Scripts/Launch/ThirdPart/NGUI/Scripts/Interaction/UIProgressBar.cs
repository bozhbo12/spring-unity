...
public class UIProgressBar : UIWidgetContainer
{
    ...
    protected DisplayMode mode
    {
        get 
        {
            if (mFill == FillDirection.LeftToRight || mFill == FillDirection.RightToLeft)
                return DisplayMode.Horizontal;
            else if (mFill == FillDirection.Radial360)
                return DisplayMode.Radial360;
    ...
    protected float Radius
    {
        get
        {
            ...
            {
                //获取半径
                UIWidget w = mBG.GetComponent<UIWidget>();
                if (w != null)
                    radius = w.width * 0.5f;
                if (thumb != null)
                {
                    w = thumb.GetComponentInChildren<UIWidget>();
                    if (w != null)
                        radius += w.width * 0.5f;
                    radius = radius - 3f;
                }
    ...
    public virtual void ForceUpdate()
    {
        ...
        if (mFG != null)
        {
            ...
            else if (mode == DisplayMode.Radial360)
            {
                if (sprite != null && sprite.type == UISprite.Type.Filled)
                {
                    sprite.fillDirection = UISprite.FillDirection.Radial360;
                    sprite.invert = isInverted;
                    sprite.fillAmount =1.0f- value;
                }
                else
                {
                    mFG.drawRegion = isInverted ?
    ...
    protected void SetThumbPosition360()
    {
        float v = (360 - value * 360) * (Mathf.PI / 180);

        Vector3 pos = new Vector3 { x = Mathf.Sin(v) * Radius, y = Mathf.Cos(v) * Radius, z = transform.position.z };

        var worldPos = transform.parent.TransformPoint(pos);
