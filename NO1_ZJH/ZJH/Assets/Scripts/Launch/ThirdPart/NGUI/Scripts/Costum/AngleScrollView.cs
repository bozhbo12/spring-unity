...
[AddComponentMenu("NGUI/Interaction/Angle Scroll View")]
public class AngleScrollView : MonoBehaviour
{
    static public BetterList<AngleScrollView> list = new BetterList<AngleScrollView>();
    ...
    private bool mSpring = false;
    public bool isSpring { get { return mSpring; } }

    private bool mRuning = false;
    public bool isRuning { get { return mRuning; } }

    public bool Run { get { return isDragging || isSpring || isRuning; } }
    ...
    public virtual Bounds bounds
    {
        get
        {
            if (!mCalculatedBounds)
            {
                ...
                min.x = mBounds.min.x - distance;
                min.y = mBounds.min.y;
                min.z = mBounds.min.z;

                Vector3 max = Vector3.zero;
                max.x = mBounds.max.x + distance;
                max.y = mBounds.max.y;
                max.z = mBounds.max.z;
    ...
    protected virtual bool shouldMove
    {
        get
        {
            ...
            float hx = (clip.z == 0f) ? ResolutionConstrain.Instance.width : clip.z * 0.5f;
            float hy = (clip.w == 0f) ? ResolutionConstrain.Instance.height : clip.w * 0.5f;
    ...
    void LateUpdate()
    {
        ...
        if (mShouldMove && !mPressed)
        {
            if (movement == Movement.Horizontal)
            {
                ...
                vTemp.x = mScroll * 0.05f;

                mMomentum -= mTrans.TransformDirection(vTemp);
            }
            else if (movement == Movement.Vertical)
            {
                Vector3 vTemp = Vector3.zero;
                vTemp.y = mScroll * 0.05f;
                mMomentum -= mTrans.TransformDirection(vTemp);
            }
            else if (movement == Movement.Unrestricted)
            {
            ...
            {
                Vector3 vTemp = Vector3.zero;
                vTemp.x = mScroll * customMovement.x * 0.05f;
                vTemp.y = mScroll * customMovement.y * 0.05f;
                mMomentum -= mTrans.TransformDirection(vTemp);
            }
            ...
            if (mMomentum.magnitude > 0.0001f)
            {
                ...
                if (mMomentum.magnitude < 0.001f && onDragFinished != null)
                {
                    //移动完成后 调用滑动回调
                    onDragFinished(5);
                    mRuning = false;
    ...
    void OnDrawGizmos()
    {
        if (mPanel != null)
        {
            ...
            Gizmos.DrawWireCube(vTemp, vTemp2);
        }
    }
#endif
}
