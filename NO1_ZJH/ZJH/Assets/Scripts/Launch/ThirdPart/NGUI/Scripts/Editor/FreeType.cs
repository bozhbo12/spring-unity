...
static public class FreeType
{
			...
			if (!mFound)
			{
				...
				else
				{
					...
					if (File.Exists(path))
					{
						...
						{
							LogSystem.LogError("Unable to copy FreeType.dylib to /usr/local/lib:\n" , ex.ToString());
						}
					}
				}
			}
	...
	static public string[] GetFaces (Font font)
	{
		...
		{
			LogSystem.LogWarning("Failed to initialize FreeType");
			return null;
		}
		...
		if (File.Exists(fileName))
		{
			...
			{
				LogSystem.LogWarning("Unable to use the chosen font (FT_New_Face).");
			}
			else
			{
	...
	static public bool CreateFont (Font ttf, int size, int faceIndex, string characters, out BMFont font, out Texture2D tex)
	{
		...
		if (!File.Exists(fileName))
		{
			LogSystem.LogWarning("Unable to use the chosen font.");
		}
		...
		else
		{
			...
			{
				// DestroyTimer the texture now that it's a part of an atlas
