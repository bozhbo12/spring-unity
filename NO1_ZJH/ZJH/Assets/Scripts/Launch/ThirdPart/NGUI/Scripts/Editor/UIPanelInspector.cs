            ...
            case EventType.Repaint:
                {
                    ...
                    if ((v2 - v0).magnitude > 60f)
                    {
                        ...
                        Handles.BeginGUI();
                        {
                            ...
                            {
                                if (!mPanel.leftAnchor.hasTarget || mPanel.leftAnchor.absolute != 0)
                                    DrawKnob(handles[4], id, resizable[4]);

                                if (!mPanel.rightAnchor.hasTarget  || mPanel.rightAnchor.absolute != 0)
                                    DrawKnob(handles[6], id, resizable[6]);
                            }

                            if (Mathf.Abs(v3.x - v0.x) > 80f)
                            {
                                if (!mPanel.topAnchor.hasTarget  || mPanel.topAnchor.absolute != 0)
                                    DrawKnob(handles[5], id, resizable[5]);

                                if (!mPanel.bottomAnchor.hasTarget  || mPanel.bottomAnchor.absolute != 0)
