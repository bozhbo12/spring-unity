...
static public class NGUIMath
{
	...
	[System.Diagnostics.DebuggerHidden]
	[System.Diagnostics.DebuggerStepThrough]
    static public int HexToDecimal2(char ch)
    {
        switch (ch)
        {
            case '0': return 0x0;
    ...
    /// <param name="considerInactive"></param>
    /// <returns></returns>
    static public Bounds CalculateRelativeWidgetBounds2(Transform relativeTo, Transform content, bool considerInactive=false)
    {
        if (content != null)
        {
            UIWidget[] widgets = content.GetComponentsInChildren<UIWidget>(considerInactive);
            ...
            if (widgets.Length > 0)
            {
                ...
                for (int i = 0, imax = widgets.Length; i < imax; ++i)
                {
                    UIWidget w = widgets[i];
                    if (!considerInactive && !w.enabled) continue;
                    if (content == w.transform) continue;
                    Vector3[] corners = w.worldCorners;
	...
	static public void MoveRect (UIRect rect, float x, float y)
	{
        ...
        if (rect.leftAnchor.hasTarget)
        {
            ++anchorCount;
            rect.leftAnchor.absolute += ix;
        }

        if (rect.rightAnchor.hasTarget)
        {
            ++anchorCount;
            rect.rightAnchor.absolute += ix;
        }

        if (rect.bottomAnchor.hasTarget)
        {
            ++anchorCount;
            rect.bottomAnchor.absolute += iy;
        }

        if (rect.topAnchor.hasTarget)
        {
            ++anchorCount;
		...
		int minWidth, int minHeight, int maxWidth, int maxHeight)
	{
		...
		if (w.isAnchored)
		{
			...
			if (w.leftAnchor.hasTarget) w.leftAnchor.SetHorizontal(t, x);
			if (w.rightAnchor.hasTarget) w.rightAnchor.SetHorizontal(t, x + finalWidth);
			if (w.bottomAnchor.hasTarget) w.bottomAnchor.SetVertical(t, y);
			if (w.topAnchor.hasTarget) w.topAnchor.SetVertical(t, y + finalHeight);
		}
    ...
    #else
			    if (platform == RuntimePlatform.BlackBerryPlayer)
    #endif
			      dpi = 160f;
#elif UNITY_WP8
