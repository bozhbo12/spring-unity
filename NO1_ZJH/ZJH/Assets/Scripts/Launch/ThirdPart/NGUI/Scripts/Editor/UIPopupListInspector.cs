...
public class UIPopupListInspector : UIWidgetContainerEditor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		{
			RegisterUndo();
			string[] split = modified.Split(SplitChars.chEnter, System.StringSplitOptions.RemoveEmptyEntries);
			mList.items.Clear();
			foreach (string s in split) mList.items.Add(s);

			if (string.IsNullOrEmpty(mList.value) || !mList.items.Contains(mList.value))
			{
				mList.value = mList.items.Count > 0 ? mList.items[0] : string.Empty;
			}
		}
        ...
        NGUIEditorTools.DrawProperty("Center", serializedObject, "isCenter");
	...
	void DrawAtlas()
	{
		if (NGUIEditorTools.DrawHeader("Atlas"))
		{
			...
			GUILayout.BeginHorizontal();
			{
				...
				NGUIEditorTools.DrawProperty(string.Empty, serializedObject, "atlas");
			}
			GUILayout.EndHorizontal();
	...
	void DrawFont ()
	{
		if (NGUIEditorTools.DrawHeader("Font"))
		{
			...
			GUILayout.BeginHorizontal();
			{
				...
				if (mType == FontType.Bitmap)
				{
					NGUIEditorTools.DrawProperty(string.Empty, serializedObject, "bitmapFont", GUILayout.MinWidth(40f));
				}
				else
				{
					ttf = NGUIEditorTools.DrawProperty(string.Empty, serializedObject, "trueTypeFont", GUILayout.MinWidth(40f));
				}
			}
			GUILayout.EndHorizontal();
			...
			if (ttf != null && ttf.objectReferenceValue != null)
			{
				GUILayout.BeginHorizontal();
				{
					...
					NGUIEditorTools.DrawProperty(string.Empty, serializedObject, "fontStyle", GUILayout.MinWidth(40f));
					GUILayout.Space(18f);
