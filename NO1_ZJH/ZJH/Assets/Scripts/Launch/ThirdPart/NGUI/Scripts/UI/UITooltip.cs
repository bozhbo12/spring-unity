	...
	protected virtual void SetText (string tooltipText)
	{
		if (text != null && !string.IsNullOrEmpty(tooltipText))
		{
			...
			if (uiCamera != null)
			{
                ...
                mPos.x = Mathf.Clamp01(mPos.x / ResolutionConstrain.Instance.width);
                mPos.y = Mathf.Clamp01(mPos.y / ResolutionConstrain.Instance.height);
                ...
                float ratio = (ResolutionConstrain.Instance.height * 0.5f) / activeSize;

				// Calculate the maximum on-screen size of the tooltip window
                Vector2 max = Vector2.zero;
                max.x =ratio * mSize.x / ResolutionConstrain.Instance.width;
                max.y = ratio * mSize.y / ResolutionConstrain.Instance.height;
			...
			else
			{
                ...
                if (mPos.x + mSize.x > ResolutionConstrain.Instance.width) mPos.x = ResolutionConstrain.Instance.width - mSize.x;
				if (mPos.y - mSize.y < 0f) mPos.y = mSize.y;

				// Simple calculation that assumes that the camera is of fixed size
                mPos.x -= ResolutionConstrain.Instance.width * 0.5f;
                mPos.y -= ResolutionConstrain.Instance.height * 0.5f;
			}
		}
		else mTarget = 0f;
	}
