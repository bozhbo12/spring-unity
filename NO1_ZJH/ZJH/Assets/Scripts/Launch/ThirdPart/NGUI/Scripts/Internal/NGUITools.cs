...
static public class NGUITools
{
    ...
    public static OnPlaySoundEvent moSoundLisnter = null;
    public static void SetPlaySoundEvent(OnPlaySoundEvent onSoundEvent)
    {
        moSoundLisnter = onSoundEvent;
    }

    static public void PlaySound(int index)
    {
        if (moSoundLisnter != null)
        {
            moSoundLisnter(index);
        }
    }
    ...
    static public AudioSource PlaySound(AudioClip clip, float volume, float pitch)
    {
        ...
        if (clip != null && volume > 0.01f)
        {
            if (mListener == null || !NGUITools.GetActive(mListener))
            {
                ...
                if (mListener == null)
                {
                    GameObject game = GameObject.Find("GameControl");
                    if (game != null)
                    {
                        mListener = game.gameObject.AddComponent<AudioListener>();
                    }
    ...
    {
#if UNITY_FLASH
		LogSystem.LogWarning("WWW is not yet implemented in Flash");
		return null;
#else
        WWW www = null;
        try { www = new WWW(url); }
        catch (System.Exception ex) { LogSystem.LogError(ex.ToString()); }
        return www;
#endif
    }
    ...
    static public WWW OpenURL(string url, WWWForm form)
    {
        ...
        catch (System.Exception ex) { LogSystem.LogError(ex != null ? ex.ToString() : "<null>"); }
        return www;
#endif
    }
    ...
    static public Camera FindCameraForLayer(int layer)
    {
        ...
        //5:UI 15:UIMain 25:UIPrompt 26:UIEffect 19:UIModel
        //if (layer == 5 || layer == 15 || layer == 25 || layer == 26 || layer == 19)
        //{
    ...
    static public BoxCollider AddWidgetCollider(GameObject go, bool considerInactive)
    {
        if (go != null)
        {
            ...
            if (box == null)
            {
                if (col != null)
                {
                    if (Application.isPlaying) CacheObjects.DestroyComponent(col);
                    else CacheObjects.DestroyComponentImmediate(col);
                }
            ...
            UpdateWidgetCollider(widget,box, considerInactive);
            return box;
        }
        return null;
    }
    ...
    static public void UpdateWidgetCollider(UIWidget widget,BoxCollider box)
    {
        UpdateWidgetCollider( widget, box, false);
    }
...
//     static public void UpdateWidgetCollider(UIWidget widget, BoxCollider box, bool considerInactive)
//     {
//         if (box != null )
//         {
//             UpdateWidgetCollider(widget, box, considerInactive);
...
//     }
    ...
    static public void UpdateWidgetCollider(UIWidget widget, BoxCollider box, bool considerInactive)
    {
        if (box != null)
        {
            ...
            //UIWidget wwidget = go.GetComponent<UIWidget>();

            if (widget != null)
            {
                Vector4 region = widget.drawingDimensions;
                Vector3 vTemp = Vector3.zero;
                vTemp.x = (region.x + region.z) * 0.5f;
                vTemp.y = (region.y + region.w) * 0.5f;
                box.center = vTemp;

                vTemp.x = region.z - region.x;
                vTemp.y = region.w - region.y;
                box.size = vTemp;
            }
            else
            {
    ...
    {
        GameObject go = CacheObjects.InstantiateClone(prefab) as GameObject;
    ...
    static public UIPanel CreateUI(Transform trans, bool advanced3D, int layer)
    {
        ...
        if (panel == null)
        {
            ...
            if (advanced3D)
            {
                ...
                Vector3 vTemp = Vector3.zero;
                vTemp.z = -700f;
                cam.transform.localPosition = vTemp;
            }
            else
            {
            ...
            if (listeners == null || listeners.Length == 0)
            {
                ...
                if (game != null)
                {
                    game.AddComponent<AudioListener>();
                }
    ...
    static public void DestroyNGUI(UnityEngine.Object obj)
    {
        //symbolInput       自生成tex
        //UIinput           自生成tex
        //uiAtlasmaker      编译器
        //uiDragDropItem    clone
        if (obj != null)
        {
            if (Application.isPlaying)
            {
                //1.texture基本是自己new出来的，无法用缓存删除
                //2.gameobject DropItem 也是通用Clone出来的，所以需用DesotryClone删除
                if (obj is GameObject)
                {
                    GameObject go = obj as GameObject;
                    go.transform.parent = null;
                    CacheObjects.DestroyClone(obj as GameObject);
                }
                else
                {
                    //避免出现泄露，多做一次判断
                    if (CacheObjects.InCacheAsset(obj))
                    {
                        CacheObjects.PopCache(obj);
                    }
    ...
    static public void DestroyImmediateNGUI(UnityEngine.Object obj)
    {
        if (obj != null)
        {
    ...
    static public void SetNotClidrenActive(GameObject go, bool state) { SetActive(go, state, false); }
    ...
    {
        UIWidget w = UIRect.GetUIRect<UIWidget>(t);//.GetComponent<UIWidget>();
    ...
    private static int g_UILayer1 = LayerMask.NameToLayer("UI");
    private static int g_UILayer2 = LayerMask.NameToLayer("UIEffect");
    private static int g_UILayer3 = LayerMask.NameToLayer("Effect");
    ...
    public static void SetLayer(GameObject go, int layer)
    {
        ...
        {
            if (go.layer == g_UILayer2 && layer == g_UILayer1)
            {
                NGUITools.AdjustParticlesToWorld(go);
            }

            int iLayer = go.layer;
            if (iLayer != g_UILayer2 && iLayer != g_UILayer3)
            {
                go.layer = layer;
            }
    ...
    struct ParticleInfo
    {
        ...
        public static ParticleInfo Zero = new ParticleInfo(0.0f, 0.0f, 0, 0.0f, 1f);
        public float fScaleRate;
        public ParticleInfo(float fStartSize, float fStartSpeed, int iMaxParticles, float fMaxParticleSize, float fScaleRate)
        {
    ...
    static Dictionary<int, ParticleInfo> mAdjustTransDict = new Dictionary<int, ParticleInfo>();
    public static void AdjustParticlesToUI(GameObject oModel, float scaleRate)
    {
        ...
        for (int i = 0; i < particles.Length; i++)
        {
            ...
            if (particle != null)
            {
                ...
                ParticleInfo pInfo = ParticleInfo.Zero;
                pInfo.fStartSize = particle.startSize;
                pInfo.fStartSpeed = particle.startSpeed;
                pInfo.iMaxParticles = particle.maxParticles;
                pInfo.fScaleRate = scaleRate;
                ...
                if (render)
                {
                    pInfo.fMaxParticleSize = render.maxParticleSize;
                    render.maxParticleSize *= scaleRate;
                }
                int iHashCode = particle.transform.GetHashCode();
                if (!mAdjustTransDict.ContainsKey(iHashCode))
                {
                    mAdjustTransDict.Add(iHashCode, pInfo);
                }
        ...
        if (particles != null)
        {
            for (int i = 0; i < particles.Length; i++)
            {
                ...
                if (particle.transform.gameObject.layer == LayerMask.NameToLayer("UIEffect"))
                {
                    ...
                    if (mAdjustTransDict.ContainsKey(iHashCode))
                    {
                        ParticleInfo pInfo = mAdjustTransDict[iHashCode];
                        particle.startSize = pInfo.fStartSize;
                        particle.startSpeed = pInfo.fStartSpeed;
                        particle.maxParticles = pInfo.iMaxParticles;
                        ParticleSystemRenderer render = particle.GetComponent<ParticleSystemRenderer>();
                        if (render)
                        {
                            render.maxParticleSize = pInfo.fMaxParticleSize;
                        }

                        mAdjustTransDict.Remove(iHashCode);
                    }
    ...
    static public Vector3[] GetSides(this Camera cam, float depth, Transform relativeTo)
    {
        ...
        vTemp.y = 0.5f;
        vTemp.z = depth;
        mSides[0] = cam.ViewportToWorldPoint(vTemp);
        vTemp.x = 0.5f;
        vTemp.y = 1f;
        vTemp.z = depth;
        mSides[1] = cam.ViewportToWorldPoint(vTemp);
        vTemp.x = 1f;
        vTemp.y = 0.5f;
        vTemp.z = depth;
        mSides[2] = cam.ViewportToWorldPoint(vTemp);
        vTemp.x = 0.5f;
        vTemp.y = 0f;
        vTemp.z = depth;
        mSides[3] = cam.ViewportToWorldPoint(vTemp);
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public static string GetViewPath(GameObject obj)
    {
        ...
        {
            trans = trans.parent;
            strName = trans.name + "/" + strName;
            if (trans.CompareTag("UIEvent"))
                break;
        }
        return strName;
    }
