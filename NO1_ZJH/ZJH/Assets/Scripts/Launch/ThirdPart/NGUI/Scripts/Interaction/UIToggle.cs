...
public class UIToggle : UIWidgetContainer
{
    ...
    void Start()
    {
        ...
        if (!Application.isPlaying)
        {
			...
			if (NormalPiction != null && NormalPic == null)
			{
				NormalPic = NormalPiction;
				NormalPiction = null;
			}
            ...
            if (Application.isPlaying && activeSprite != null)
				activeSprite.transform.gameObject.SetActive (startsActive);

			if (Application.isPlaying && NormalPic != null) 
			{
				NormalPic.transform.gameObject.SetActive (!startsActive);
			}
    ...
    void Set(bool state)
    {
        if (!mStarted)
        {
            mIsActive = state;
            startsActive = state;
			if (activeSprite != null) activeSprite.transform.gameObject.SetActive (state);
			if (NormalPic != null) NormalPic.transform.gameObject.SetActive (!state);
        }
        else if (mIsActive != state)
        {
            ...
            if (activeSprite != null)
            {
                ...
                {
					activeSprite.transform.gameObject.SetActive (mIsActive);
                }
            }
			if (NormalPic != null)
			{
				NormalPic.transform.gameObject.SetActive (!mIsActive);
			}
