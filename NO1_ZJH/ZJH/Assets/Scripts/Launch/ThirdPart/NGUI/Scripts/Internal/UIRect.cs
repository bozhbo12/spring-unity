...
public abstract class UIRect : MonoBehaviour
{
	...
	public class AnchorPoint
	{
		...
		public Vector3[] GetSides (Transform relativeTo)
		{
			if (hasTarget)
			{
                ...
                Camera camera = tranTarget.GetComponent<Camera>();
                if (camera != null)
                {
                    return camera.GetSides(relativeTo);
                }
			}
			return null;
		}
    ...
    private bool mbHasGo = false;
    private GameObject mGoOrgin = null;
	protected GameObject mGo
    {
        set
        {
            mGoOrgin = value;
            mbHasGo = mGoOrgin != null;
        }
        get
        {
            return mGoOrgin;
    ...
    private bool mbHasTrans = false;
    private Transform mTransOrgin = null;
    protected Transform mTrans
    {
        set
        {
            mTransOrgin = value;
            mbHasTrans = mTransOrgin != null;
        }
        get
        {
            return mTransOrgin;
	...
	UIRoot mRoot;
    UIRect mParent = null;
    protected bool mbHasParent = false;
    protected UIRect ParentPanel
    {
        set
        {
            mParent = value;
            mbHasParent = value != null; 
        }
        get
        {
            return mParent;
	...
	public GameObject cachedGameObject { get { if (!mbHasGo) mGo = gameObject; return mGo; } }

	/// <summary>
	/// Transform gets cached for speed. Can't simply return 'mTrans' set in Awake because this function may be called on a prefab.
	/// </summary>

	public Transform cachedTransform { get { if (!mbHasTrans) mTrans = transform; return mTrans; } }
	...
	public bool isFullyAnchored { get { return leftAnchor.hasTarget && rightAnchor.hasTarget && topAnchor.hasTarget && bottomAnchor.hasTarget; } }

	/// <summary>
	/// Whether the rectangle is anchored horizontally.
	/// </summary>

	public virtual bool isAnchoredHorizontally { get { return leftAnchor.hasTarget || rightAnchor.hasTarget; } }

	/// <summary>
	/// Whether the rectangle is anchored vertically.
	/// </summary>

	public virtual bool isAnchoredVertically { get { return bottomAnchor.hasTarget || topAnchor.hasTarget; } }
	...
	public UIRect parent
	{
		get
		{
			...
			{
				mParentFound = true;
				ParentPanel = FindInParents<UIRect>(cachedTransform.parent);
			}
			return ParentPanel;
		}
	}
	...
	public UIRoot root
	{
		get
		{
			if (mbHasParent) return ParentPanel.root;
	...
	public bool isAnchored
	{
		get
		{
			return (leftAnchor.hasTarget || rightAnchor.hasTarget || topAnchor.hasTarget || bottomAnchor.hasTarget) && canBeAnchored;
		}
	}
	...
	protected Vector3 GetLocalPos (AnchorPoint ac, Transform trans)
	{
		...
		Vector3 pos = mMyCam.ViewportToWorldPoint(ac.targetCam.WorldToViewportPoint(ac.tranTarget.position));
		if (trans != null) pos = trans.InverseTransformPoint(pos);
		pos.x = Mathf.Floor(pos.x + 0.5f);
		pos.y = Mathf.Floor(pos.y + 0.5f);
		return pos;
	}
    private static Dictionary<Transform, UIRect> mVisiblePanelTrans = new Dictionary<Transform, UIRect>();
    public static void PushPanel(UIRect panel)
    {
        if (panel == null)
            return;

        if (!mVisiblePanelTrans.ContainsKey(panel.transform))
        {
            mVisiblePanelTrans.Add(panel.transform, panel);
        }
    }
    public static void PopPanel(UIRect panel)
    {
        if (panel == null)
            return;
        ...
        if (mVisiblePanelTrans.ContainsKey(panel.transform))
        {
            mVisiblePanelTrans.Remove(panel.transform);
        }
    }
    public static T GetUIRect<T>(Transform trans) where T : UIWidget
    {
        //如果当前对象包含脚步对象
        if (mVisiblePanelTrans.ContainsKey(trans))
        {
            UIRect panel = mVisiblePanelTrans[trans];
            return panel as T;
        }
        return null;
    }
    public static T FindInParents<T>(Transform trans) where T : UIRect
    {
        if (trans == null)
            return null;
        ...
        UIRect panel;
        //如果当前对象包含脚步对象
        if (mVisiblePanelTrans.TryGetValue(trans, out panel))
        {
        ...
        else
        {
            ...
            while (t != null)
            {
                if (mVisiblePanelTrans.TryGetValue(t, out panel))
                {
                    if (panel != null && panel.gameObject.activeInHierarchy)
                    {
                        return panel as T;
	...
	protected virtual void OnInit ()
	{
		...
		mRootSet = false;
		mParentFound = false;
		if (parent != null) ParentPanel.mChildren.Add(this);
	}
    void OnDestroy()
    {
        if (this is UIPanel)
	...
	{
		if (ParentPanel) ParentPanel.mChildren.Remove(this);
        ParentPanel = null;
		mRoot = null;
		mRootSet = false;
		mParentFound = false;
	...
	{
		leftAnchor.tranTarget = t;
		rightAnchor.tranTarget = t;
		topAnchor.tranTarget = t;
		bottomAnchor.tranTarget = t;
	...
	public void ResetAnchors ()
	{
		...
		leftAnchor.rect		= (leftAnchor.hasTarget)	? leftAnchor.tranTarget.GetComponent<UIRect>()	 : null;
		bottomAnchor.rect	= (bottomAnchor.hasTarget) ? bottomAnchor.tranTarget.GetComponent<UIRect>() : null;
		rightAnchor.rect	= (rightAnchor.hasTarget)	? rightAnchor.tranTarget.GetComponent<UIRect>()	 : null;
		topAnchor.rect		= (topAnchor.hasTarget)	? topAnchor.tranTarget.GetComponent<UIRect>()	 : null;
	...
	void FindCameraFor (AnchorPoint ap)
	{
		...
		else
		{
			...
			ap.targetCam = NGUITools.FindCameraForLayer(ap.tranTarget.gameObject.layer);
		}
	}
	...
	{
		mParentFound = false;
		UIRect pt = FindInParents<UIRect>(cachedTransform.parent);

		if (ParentPanel != pt)
		{
			if (ParentPanel) ParentPanel.mChildren.Remove(this);
            ParentPanel = pt;
			if (ParentPanel) ParentPanel.mChildren.Add(this);
			mRootSet = false;
		}
	}
