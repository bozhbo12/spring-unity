...
public static class BMFontReader
{
	...
	{
		int idx = s.IndexOf('=');
		return (idx == -1) ? string.Empty : s.Substring(idx + 1);
	}
	...
	static public void Load (BMFont font, string name, byte[] bytes)
	{
		...
		if (bytes != null)
		{
			...
			char[] separator = SplitChars.chSpace;
			...
			while (reader.canRead)
			{
				...
				if (split[0] == "char")
				{
					...
					{
						LogSystem.LogWarning("Your font was exported with more than one texture. Only one texture is supported by NGUI.\n" ,
					...
					if (len > 8)
					{
						...
						else LogSystem.LogWarning("Char: " , split[1] , " (" , id , ") is NULL");
					}
					else
					{
						LogSystem.LogWarning("Unexpected number of entries for the 'char' field (" , name , ", " ,split.Length , "):\n" , line);
						break;
					}
				}
				else if (split[0] == "kerning")
				{
					...
					{
						LogSystem.LogWarning("Unexpected number of entries for the 'kerning' field (" ,
							name , ", " , split.Length, "):\n" ,line);
						break;
					}
				}
				else if (split[0] == "common")
				{
					...
					if (len > 5)
					{
						...
						if (pages != 1)
						{
							LogSystem.LogWarning("Font '" , name , "' must be created with only 1 texture, not " , pages);
							break;
						}
					}
					else
					{
						LogSystem.LogWarning("Unexpected number of entries for the 'common' field (" ,
							name , ", " , split.Length , "):\n" , line);
						break;
					}
				}
				else if (split[0] == "page")
				{
					...
					if (len > 2)
					{
						font.spriteName = GetString(split[2]).Replace("\"", string.Empty);
