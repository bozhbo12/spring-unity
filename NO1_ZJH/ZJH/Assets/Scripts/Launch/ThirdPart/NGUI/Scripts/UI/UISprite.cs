...
public class UISprite : UIWidget
{
    ...
    [HideInInspector]
    [SerializeField]
    string mstrAtlasName = string.Empty; ///图集对应的资源路径 Resources/Prefabs/UIAtlas/
    [HideInInspector]
    UIAtlas mAtlas = null;
    ...
    public string AtlasName
    {
        get
        {
            return mstrAtlasName;
        }
        set
        {
			
            if (mstrAtlasName != value)
            {
                ///重新加载图集
                if (mstrLoadAtlasName != mstrAtlasName)
                {
                    mstrLoadAtlasName = value;
                    LoadAtlas(value);
                }
                mstrAtlasName = value;
            }
        ...
        set
        {
            if (mAtlas != value)
            {
                ...
                if (!Application.isPlaying)
                {
                    ...
                    if (mstrAtlasName != strAtlasPath)
                    {
                        mstrAtlasName = strAtlasPath;
                    }
                }
#endif
    ...
    public override bool CheckWaitLoadingAtlas()
    {
        ...
        return !string.IsNullOrEmpty(AtlasName) && atlas == null;
    }
    ...
    public override void CheckLoadAtlas()
    {
        ...
        if (atlas == null && !string.IsNullOrEmpty(AtlasName))
        {
            ...
            UIAtlas uiAtlas = Config.GetAtlas(AtlasName);
            if (uiAtlas != null)
            {
                atlas = uiAtlas;
                MarkAsChanged();
            ...
            {
                if (mstrLoadAtlasName != AtlasName)
                {
                    mstrLoadAtlasName = AtlasName;
                    LoadAtlas(AtlasName);
                }
            }
        }
    ...
    static private string strAtlasPath =  "Prefabs/UIAtlas/";
    static private string strPathSplit = "/";
    /// <summary>
    /// 加载对应的图集资源
    /// </summary>
    ...
    private void LoadAtlas(string strAtlasName)
    {
        if (Application.isPlaying)
        {
            //string strFileName = UtilTools.StringBuilder(strAtlasPath, Language.LanguageVersion, strPathSplit, strAtlasName);
            if (strAtlasName.StartsWith("Local/"))
            {
                if (monLoadAsset != null)
                {
                    monLoadAsset(strAtlasName, OnFileLoaded,null);
                }
                else
                {
                    ///编辑器模式
                    Object oAsset = Resources.Load(strAtlasName);
                    CacheObjects.PushCache(strAtlasName, oAsset);
                    OnFileLoaded(oAsset, strAtlasName, null);
                }
            }
            else
            {
                string strFileName = strAtlasPath +strPathSplit + strAtlasName;
                if (monLoadAsset != null)
                {
        ...
        else
        {
            ...
            if (strAtlasName.StartsWith("Local"))
            {
                strFileName = "Assets/Resources/" + strAtlasName + ".prefab";
            }
            else
            {
                strFileName = "Assets/Resources/Prefabs/UIAtlas/" +  "/" + strAtlasName + ".prefab";
            }
    ...
    private void OnFileLoaded(UnityEngine.Object oAsset2, string strFileName, VarStore varStore = null)
    {
        ...
        if (oAsset == null)
        {
            error = true;
#if UNITY_EDITOR
            if (!string.IsNullOrEmpty(mstrAtlasName))
            {
                LogSystem.LogWarning("加载 图集失败---- 图集名字" + mstrAtlasName + "   UISprite名字" + this.name);
            }
#endif
            //if (monLoaded != null)
            //{
    ...
    protected override void OnInit()
    {
        ...
        if (!string.IsNullOrEmpty(mstrAtlasName) )
        {
            UIAtlas uiAtlas = Config.GetAtlas(mstrAtlasName);
            if (uiAtlas == null)
            {
                if (mstrLoadAtlasName != mstrAtlasName)
                {
                    mstrLoadAtlasName = mstrAtlasName;
                    ///加载对应的材质球
                    LoadAtlas(mstrAtlasName);
                }
            }
            else
            {
...
//         cols.Add(col);
    ...
    protected void FilledFill(List<Vector3> verts, List<Vector2> uvs, List<Color32> cols)
    {
    ...
    protected void AdvancedFill(List<Vector3> verts, List<Vector2> uvs, List<Color32> cols)
    {
        if (!mSprite.hasBorder)
        {
            SimpleFill(verts, uvs, cols);
    ...
    /// 功能 解决挖空遮罩
    /// </summary>
    protected void CustomFill(List<Vector3> verts, List<Vector2> uvs, List<Color32> cols)
    {
        Texture tex = material.mainTexture;
        if (tex == null)
            return;
    ...
    void SetUVandVert(float left, float right, float bottom, float top, float u0, float v0, float u1, float v1, Color32 col, List<Vector3> verts, List<Vector2> uvs, List<Color32> cols)
    {
        Vector3 vTemp = Vector3.zero;
        vTemp.x = left;
        vTemp.y = bottom;
    ...
    void FillBuffers(float v0x, float v1x, float v0y, float v1y, float u0x, float u1x, float u0y, float u1y, Color col,
        List<Vector3> verts, List<Vector2> uvs, List<Color32> cols)
    {
        Vector3 vTemp = Vector3.zero;
        vTemp.x = v0x;
        vTemp.y = v0y;
