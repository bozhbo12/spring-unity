...
public class UISpriteAnimationInspector : Editor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		string namePrefix = EditorGUILayout.TextField("Name Prefix", (anim.namePrefix != null) ? anim.namePrefix : string.Empty);
        ...
        bool perfect = EditorGUILayout.Toggle("MakePixelPerfect", anim.perfect);
        if (anim.perfect != perfect)
        {
            NGUIEditorTools.RegisterUndo("Sprite Animation Change", anim);
            anim.perfect = perfect;
            EditorUtility.SetDirty(anim);
        }
	}
}