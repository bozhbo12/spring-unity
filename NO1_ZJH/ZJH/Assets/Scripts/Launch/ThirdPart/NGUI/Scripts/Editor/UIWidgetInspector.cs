...
public class UIWidgetInspector : UIRectEditor
{
    ...
    static public void DrawAnchorHandle(UIRect.AnchorPoint anchor, Transform myTrans, Vector3[] myCorners, int side, int id)
    {
        ...
        {
            Camera camera = anchor.tranTarget.GetComponent<Camera>();
            if (camera != null)
            {
                sides = camera.GetWorldCorners();
            }
        }
        ...
        {
            theirPos = anchor.tranTarget.position;
        }
            ...
            case EventType.Repaint:
                {
                    ...
                    if ((v2 - v0).magnitude > 60f)
                    {
                        ...
                        Handles.BeginGUI();
                        {
                            ...
                            {
                                if (!mWidget.leftAnchor.hasTarget || mWidget.leftAnchor.absolute != 0)
                                    DrawKnob(handles[4], mWidget.pivot == pivotPoints[4], resizable[4], id);

                                if (!mWidget.rightAnchor.hasTarget || mWidget.rightAnchor.absolute != 0)
                                    DrawKnob(handles[6], mWidget.pivot == pivotPoints[6], resizable[6], id);
                            }

                            if ((v3 - v0).magnitude > 80f)
                            {
                                if (!mWidget.topAnchor.hasTarget || mWidget.topAnchor.absolute != 0)
                                    DrawKnob(handles[5], mWidget.pivot == pivotPoints[5], resizable[5], id);

                                if (!mWidget.bottomAnchor.hasTarget || mWidget.bottomAnchor.absolute != 0)
    ...
    protected override void DrawCustomProperties()
    {
        ...
        if (NGUIEditorTools.DrawHeader("Widget"))
        {
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                NGUIEditorTools.DrawProperty(string.Empty, aspect, false, GUILayout.MinWidth(20f));
            }
            GUILayout.EndHorizontal();
    ...
    void DrawDepth(bool isPrefab)
    {
        ...
        GUILayout.BeginHorizontal();
        {
            ...
            NGUIEditorTools.DrawProperty(string.Empty, serializedObject, "mDepth", GUILayout.MinWidth(20f));
    ...
    protected override void OnDrawFinalProperties()
    {
        ...
        if (!IsRect(sp))
        {
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Space(6f);
            NGUIEditorTools.DrawProperty(string.Empty, serializedObject, "hideIfOffScreen", GUILayout.Width(18f));
