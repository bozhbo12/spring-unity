...
public class UIAtlasMaker : EditorWindow
{
	...
	static public bool UpdateTexture (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		string oldPath = (tex != null) ? AssetDatabase.GetAssetPath(tex.GetInstanceID()) : string.Empty;
		...
		if (System.IO.File.Exists(newPath))
		{
			...
			{
                LogSystem.LogWarning(newPath, " is not editable. Did you forget to do a check out?");
				return false;
			}
#endif
		...
		if (PackTextures(tex, sprites))
		{
			...
			if (newTexture)
			{
                if (tex == null) LogSystem.LogWarning("Failed to load the created atlas saved as ", newPath);
		...
		else
		{
			...
			if (create)
			{
				...
				if (!string.IsNullOrEmpty(prefabPath))
				{
					...
					string atlasName = prefabPath.Replace(".prefab", string.Empty);
