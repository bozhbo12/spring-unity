...
public class UIDraggableCamera : MonoBehaviour
{
	...
	Vector3 CalculateConstrainOffset ()
	{
        ...
        Vector3 bottomLeft = new Vector3(mCam.rect.xMin * ResolutionConstrain.Instance.width, mCam.rect.yMin * ResolutionConstrain.Instance.height, 0f);
        Vector3 topRight = new Vector3(mCam.rect.xMax * ResolutionConstrain.Instance.width, mCam.rect.yMax * ResolutionConstrain.Instance.height, 0f);

		bottomLeft = mCam.ScreenToWorldPoint(bottomLeft);
		topRight = mCam.ScreenToWorldPoint(topRight);

        Vector2 minRect = Vector2.zero;
        minRect.x = mBounds.min.x;
        minRect.y = mBounds.min.y;
        Vector2 maxRect = Vector2.zero; 
        maxRect.x = mBounds.max.x;
        maxRect.y = mBounds.max.y;
