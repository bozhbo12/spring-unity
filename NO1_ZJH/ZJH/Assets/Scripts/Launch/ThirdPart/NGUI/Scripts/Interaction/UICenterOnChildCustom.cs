...
[AddComponentMenu("NGUI/Interaction/Center Scroll View on Child Custom")]
public class UICenterOnChildCustom : MonoBehaviour
{
    /// <summary>
    /// 
    /// </summary>
    /// <param name="centeredObject"></param>
    ...
    /// 拖动是否重新计算中心
    /// </summary>
    public bool dragFinishReCenter = true;
    ...
    /// add By shijinhao 
    /// </summary>
    public float gridItemOffset = 0f;
    ...
    void OnDragFinished() { if (enabled && dragFinishReCenter) Recenter(); }
    ...
    public void Recenter()
    {
        if (mScrollView == null)
        {
            ...
            if (mScrollView == null)
            {
                LogSystem.LogWarning(GetType(), " requires ", typeof(UIScrollView), " on a parent object in order to work", this);
                enabled = false;
                return;
            }
            else
        ...
        {
            panelCenter = corners[2];// (corners[2] + corners[0]) * 0.5f; //modified by shijinhao
        }
        else
        {
    ...
    void CenterOn(Vector3 worldTarget, Vector3 panelCenter)
    {
        if (mScrollView != null && mScrollView.panel != null)
        {
            ...
            Vector3 cp = panelTrans.InverseTransformPoint(worldTarget);
            Vector3 cc = panelTrans.InverseTransformPoint(panelCenter);
            Vector3 localOffset = cp - cc;
            if (mScrollView.movement == UIScrollView.Movement.Vertical)
            {
                localOffset.y += gridItemOffset; //add by shijinhao
            }
            if (mScrollView.movement == UIScrollView.Movement.Horizontal)
            {
                localOffset.x += gridItemOffset;
            }
    ...
    public void CenterOn(Vector3 worldTarget)
    {
        if (mScrollView != null && mScrollView.panel != null)
        {
            Vector3[] corners = mScrollView.panel.worldCorners;
            Vector3 panelCenter = (corners[2] + corners[0]) * 0.5f;
            CenterOn(worldTarget, panelCenter);
        }
    }//
    #region 释放
