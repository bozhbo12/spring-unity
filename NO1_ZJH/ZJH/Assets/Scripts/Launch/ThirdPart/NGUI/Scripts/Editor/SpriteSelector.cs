...
public class SpriteSelector : ScriptableWizard
{
    ...
    void OnGUI()
    {
        ...
        else
        {
            ...
            string after = EditorGUILayout.TextField(string.Empty, before, "SearchTextField");
            ...
            int columns = Mathf.FloorToInt(ResolutionConstrain.Instance.width / padded);
    ...
    static public void Show(SerializedObject ob, SerializedProperty pro, string atlasName)
    {
        if (instance != null)
        {
            instance.Close();
        ...
        if (ob != null && pro != null && !string.IsNullOrEmpty(atlasName))
        {
            SpriteSelector comp = ScriptableWizard.DisplayWizard<SpriteSelector>("Select a Sprite");
            UIAtlas atlas = null;
            string path = string.Empty;
            if (atlasName.StartsWith("Local/"))
            {
                path = atlasName;
            }
            else
            {
                path = "Prefabs/UIAtlas/" + "/" + atlasName;
            }
            if (Application.isPlaying)
            {
                GameObject oAsset = Resources.Load<GameObject>(path);
                if (oAsset != null)
                    atlas = oAsset.GetComponent<UIAtlas>();
            }
            else
            {
                path = "Assets/Resources/" + path + ".prefab";
                GameObject oAsset = UnityEditor.AssetDatabase.LoadMainAssetAtPath(path) as GameObject;
                if (oAsset != null)
                    atlas = oAsset.GetComponent<UIAtlas>();
            }
