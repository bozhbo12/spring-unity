...
public class UIFontInspector : Editor
{
    ...
    string mSymbolSequence = string.Empty;
    string mSymbolSprite = string.Empty;
    ...
    public override void OnInspectorGUI()
    {
        ...
        else
        {
            ...
            if (mFont.atlas != null || mFont.material != null)
            {
                ...
                if (data != null)
                {
                    ...
                    LogSystem.Log("Imported " , mFont.bmFont.glyphCount ," characters");
                }
            }
            ...
            if (mFont.atlas != null)
            {
                if (NGUIEditorTools.DrawHeader("Symbols and Emoticons"))
                {
                    ...
                    if (GUILayout.Button("Add", GUILayout.Width(40f)) && isValid)
                    {
                        ...
                        mFont.MarkAsChanged();
                        mSymbolSequence = string.Empty;
                        mSymbolSprite = string.Empty;
                    }
                    GUI.backgroundColor = Color.white;
                    GUILayout.EndHorizontal();
        ...
        if (mFont.bmFont != null && mFont.bmFont.isValid)
        {
            if (NGUIEditorTools.DrawHeader("Modify"))
            {
                ...
                mCurve = EditorGUILayout.CurveField(string.Empty, mCurve, GUILayout.Width(40f), GUILayout.Height(40f));
                GUILayout.EndHorizontal();
    ...
    void ApplyEffect(Effect effect, Color foreground, Color background)
    {
        ...
        if (mFont.atlas == null)
        {
            ...
            NGUITools.DestroyImmediateNGUI(final);
        ...
        for (int i = 0; i < glyphTextures.Count; ++i)
            NGUITools.DestroyImmediateNGUI(glyphTextures[i]);
