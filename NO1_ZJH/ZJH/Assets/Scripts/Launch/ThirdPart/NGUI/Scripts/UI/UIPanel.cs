...
public class UIPanel : UIRect
{
    ...
    /// <summary>
    /// UI鐗规晥鑷?傚簲娓叉煋闃熷垪(RenderQueue.Automatic妯″紡璋冪敤)
    /// zhangrj
    /// 20151016
    /// </summary>
    ...
    public Vector4 finalClipRegion
    {
        get
        {
            ...
            if (mClipping != UIDrawCall.Clipping.None)
            {
                ...
                vTemp.x = mClipRange.x + mClipOffset.x;
                vTemp.y = mClipRange.y + mClipOffset.y;
                vTemp.z = size.x;
                vTemp.w = size.y;
                return vTemp;
            }
    ...
    public override Vector3[] GetSides(Transform relativeTo)
    {
        if (mClipping != UIDrawCall.Clipping.None || anchorOffset)
        {
            ...
            vTemp.x = x0;
            vTemp.y = cy;
            mCorners[0] = mat.MultiplyPoint3x4(vTemp);
            vTemp.x = cx;
            vTemp.y = y1;
            mCorners[1] = mat.MultiplyPoint3x4(vTemp);
            vTemp.x = x1;
            vTemp.y = cy;
            mCorners[2] = mat.MultiplyPoint3x4(vTemp);
            vTemp.x = cx;
            vTemp.y = y0;
            mCorners[3] = mat.MultiplyPoint3x4(vTemp);
    ...
    public override float CalculateFinalAlpha(int frameID)
    {
        if (mAlphaFrameID != frameID)
        {
            ...
            finalAlpha = (mbHasParent) ? ParentPanel.CalculateFinalAlpha(frameID) * mAlpha : mAlpha;
        }
        return finalAlpha;
    }
    ...
    /// 20151016
    /// </summary>
    /// <param name="_renderQuene"></param>
    public void AddRenderQuene(AdaptiveRenderQueue _renderQuene)
    {
        if (_renderQuene == null) return;
        if (!renderQuenes.Contains(_renderQuene))
            renderQuenes.Add(_renderQuene);
        renderQuenes.Sort(AdaptiveRenderQueue.RenderQueueCompareFunc);
    }
    ...
    {
        if (_renderQuene == null) return;
        renderQuenes.Remove(_renderQuene);
        renderQuenes.Sort(AdaptiveRenderQueue.RenderQueueCompareFunc);
    }

    /// <summary>
    /// DestroyTimer all draw calls we've created when this script gets disabled.
    /// </summary>
    ...
    protected override void OnDisable()
    {
        for (int i = 0; i < drawCalls.size; ++i)
        {
            ...
            if (dc != null) UIDrawCall.DestroyDrawCall(dc);
        }
    ...
    void LateUpdate()
    {
        int iFrameCount = Time.frameCount;

        if (iFrameCount != mUpdateFrame )
        {
            mUpdateFrame = iFrameCount;
            ...
            for (int i = 0; i < list.size; ++i)
            {
                ...
                if (p.renderQueue == RenderQueue.Automatic)
                {
                    ...
                    //zhangrj
                    //20151016
                    if (p.renderQuenes.size > 0)
                    {
                        for (int j = p.renderQuenes.size - 1; j >= 0; --j)
                        {
                            if (p.renderQuenes[j] != null)
                            {
                                p.renderQuenes[j].OnUpdate(p.startingRenderQueue, p.drawCalls.size);
                                rq += (p.renderQuenes[j].GetDrawCalls + p.renderQuenes[j].mEffectDepth + 1);
                            }
                            else
                            {
                                p.renderQuenes.RemoveAt(j);
                            }
                        }
                    }
                }
                ...
                CustomRenderQueue.UpdateRendererQueue(p.transform);
//                 CustomRenderQueue[] crqList = p.GetComponentsInChildren<CustomRenderQueue>();
//                 for (int j = 0; j < crqList.Length; j++)
//                 {
//                     CustomRenderQueue crq = crqList[j];
//                     if (crq != null)
//                     {
//                         crq.UpdateRendererQueue();
//                     }
//                 }
            }
        }
    ...
    {
        for (int i = 0; i < drawCalls.size; ++i)
            UIDrawCall.DestroyDrawCall(drawCalls.buffer[i]);
        drawCalls.Clear();
        ...
        for (int i = 0; i < widgets.size; ++i)
        {
            ...
            if (w.isVisible && w.hasVertices)
            {
                ...
                if (mat != null || sdr != null || tex != null)
                {
                    if (dc == null)
                    {
                        dc = UIDrawCall.Create(this, mat, tex, sdr, w.drawCall);
                        if (dc == null)
                        {
    ...
    bool FillDrawCall(UIDrawCall dc)
    {
        if (dc != null)
        {
            ...
            for (int i = 0; i < widgets.size; )
            {
                ...
                {
#if UNITY_EDITOR
                    LogSystem.LogWarning("This should never happen");
#endif
                    widgets.RemoveAt(i);
                    continue;
                }
    ...
    void UpdateDrawCalls()
    {
        ...
        if (range.z == 0f) range.z = ResolutionConstrain.Instance.width * 0.5f;
        if (range.w == 0f) range.w = ResolutionConstrain.Instance.height * 0.5f;
    ...
    public virtual Vector3 CalculateConstrainOffset(Vector2 min, Vector2 max)
    {
        ...
        vTemp.x = min.x;
        vTemp.y = min.y;
        Vector2 minRect = vTemp;
        vTemp.x = max.x;
        vTemp.y = max.y;
        Vector2 maxRect = vTemp;
        vTemp.x = cr.x - offsetX;
        vTemp.y = cr.y - offsetY;
        Vector2 minArea = vTemp;
        vTemp.x = cr.x + offsetX;
        vTemp.y = cr.y + offsetY;
        Vector2 maxArea = vTemp;
    ...
    public bool ConstrainTargetToBounds(Transform target, bool immediate)
    {
        ...
        if (bounds.center.Equals(Vector3.zero) && bounds.size.Equals(Vector3.zero))
            return false;
    ...
    static public UIPanel Find(Transform trans, bool createIfMissing, int layer)
    {
        ...
        {
            panel = FindInParents<UIPanel>(trans);
    ...
    Vector2 GetWindowSize()
    {
        UIRoot rt = root;
#if UNITY_EDITOR
        Vector2 vSize = GetMainGameViewSize();
        if (rt != null) vSize *= rt.GetPixelSizeAdjustment(Mathf.RoundToInt(vSize.y));
#else
        Vector2 vSize = Vector2.zero;
        vSize.x = Screen.width;
        vSize.y = Screen.height;

		if (rt != null) vSize *= rt.GetPixelSizeAdjustment(Screen.height);
#endif
        return vSize;
    }
    ...
    void OnDrawGizmos()
    {
        ...
        {
            Vector3 vTemp = Vector3.zero;
            vTemp.x = mClipOffset.x + mClipRange.x;
            vTemp.y = mClipOffset.y + mClipRange.y;
            Vector3 pos = clip ? vTemp : Vector3.zero;
