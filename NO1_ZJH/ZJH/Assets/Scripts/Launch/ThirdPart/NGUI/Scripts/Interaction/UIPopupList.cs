...
public class UIPopupList : UIWidgetContainer
{
    ...
    void Highlight(UILabel lbl, bool instant)
    {
        if (mHighlight != null)
        {
            ...
            Vector3 pos = lbl.cachedTransform.localPosition + new Vector3(isCenter ? 0 : -offsetX, offsetY, 1f);
    ...
    void OnClick()
    {
        if (enabled && NGUITools.GetActive(gameObject) && mChild == null && atlas != null && isValid && items.Count > 0)
        {
            ...
            mHighlight.pivot = isCenter ? UIWidget.Pivot.Top : UIWidget.Pivot.TopLeft;
            ...
            for (int i = 0, imax = items.Count; i < imax; ++i)
            {
                ...
                if (isCenter)
                {
                    lbl.pivot = UIWidget.Pivot.Top;
                    lbl.cachedTransform.localPosition = new Vector3(mBackground.localSize.x + padding.x, y, -1f);
                }
                else
                {
            ...
            float cx = x / dynScale;
            Vector3 bcCenter = new Vector3(isCenter ? 0 : cx * 0.5f, -fontHeight * 0.5f, 0f);
