...
public class BetterList<T>
{
    ...
    public class BetterListComparer : IEqualityComparer<T>
    {
    ...
    private BetterListComparer mcomparer = new BetterListComparer();
    /// <summary>
    /// Direct access to the buffer. Note that you should not use its 'Length' parameter, but instead use BetterList.size.
    /// </summary>
    ...
    /// 扩展申请大小
    /// </summary>
    /// <param name="iExpSize"></param>
    public void Add( T[] arrayData , int iSize )
    {
        if (arrayData==null)
            return;

        int iLength = iSize > 0 ? iSize : arrayData.Length ;
        if( buffer == null )
        {
            buffer = new T[ iLength];
           
            ///拷贝当前数据
            Array.Copy(arrayData, 0, buffer, size, iLength);
            size += iLength;
        }
        else if (buffer.Length - size >= iLength)
        {
            ///足够放下
            Array.Copy(arrayData,0,buffer, size, iLength);
            size += iLength;
        ...
        {
            ///多分配一点
            T[] newList = new T[buffer.Length + iLength];
            if (buffer != null && size > 0)
            {
    ...
    public void Add(T arrayData, int iSize)
    {
        ...
        int iLength = iSize > 0 ? iSize : 0;
        if( buffer == null )
        {
            ///多分配一点
            buffer = new T[buffer.Length + iLength];
           
            ///拷贝当前数据
            for (int i = 0; i < iSize; i++)
            {
                buffer[size++] = arrayData;
            }
        }
        else if ( buffer.Length - size >= iLength)
        {
	...
	public bool Contains (T item)
	{
        ...
        for (int i = 0; i < size; ++i)
        {
            if (mcomparer.Equals(buffer[i], item))
            {
                return true;
            }
        }
