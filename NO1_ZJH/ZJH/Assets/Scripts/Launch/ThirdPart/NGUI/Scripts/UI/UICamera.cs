...
public class UICamera : MonoBehaviour
{
    ...
    //static private OnScreenResize onScreenResize;
    public static List<OnScreenResize> mOnScreenReSize = new List<OnScreenResize>();
    /// <summary>
    ...
    static UIInput mCurrentInput = null;
    ...
    /// </summary>
    void GetCurrentSelectionInput()
    {
        if (mCurrentSelection != null)
        {
            mCurrentInput = mCurrentSelection.GetComponent<UIInput>();
        }
    }
    ...
    static public void Notify(GameObject go, string funcName, object obj)
    {
        ...
        try
        {
            if (NGUITools.GetActive(go))
            {
                ...
                if (moNotifyLisnter != null)
                {
                    moNotifyLisnter(go, funcName, obj);
                }
            }
        ...
        {
            mNotifying = false;
            LogSystem.LogError("Notify",ex.ToString());
        }
    ...
    public static OnNotifyUIEvent moNotifyLisnter = null;
    public static void SetNotifyUIEvent(OnNotifyUIEvent onNotifyEvent)
    {
        moNotifyLisnter = onNotifyEvent;
    }
    /// <summary>
    ...
    void Awake()
    {
        mWidth = ResolutionConstrain.Instance.width;
        mHeight = ResolutionConstrain.Instance.height;
 ...
 || Application.platform == RuntimePlatform.BlackBerryPlayer
    ...
    void Update()
    {
        ...
        if (ResolutionConstrain.Instance.width != mWidth || ResolutionConstrain.Instance.height != mHeight)
        {
            mWidth = ResolutionConstrain.Instance.width;
            mHeight = ResolutionConstrain.Instance.height;
            if (mOnScreenReSize.Count > 0)
            {
                int iCount = mOnScreenReSize.Count;
                for (int i = 0; i < iCount; i++)
                {
                    if (mOnScreenReSize[i] != null)
                    {
                        mOnScreenReSize[i]();
                    }
                }
    ...
    public void ProcessTouches()
    {
        ...
        for (int i = 0; i < Input.touchCount; ++i)
        {
            ...
            /*start 3d Touch*/
            currentTouch.deltaPressure = pressed ? touch.pressure - currentTouch.pressure : 0f;
            currentTouch.pressure = touch.pressure;
            currentTouch.maximumPossiblePressure = touch.maximumPossiblePressure;
            /*end*/
            ...
            // Process the events from this 3Dtouch
            ProcessTouch(pressed, unpressed);
    ...
    public void ProcessOthers()
    {
        ...
        inputHasFocus = (mCurrentSelection != null && mCurrentInput != null);
    ...
    public void ProcessTouch(bool pressed, bool unpressed)
    {
        ...
        if (Input.touchPressureSupported)
        {
            if (unpressed)
            {
                if (currentTouch.enter)
                {
                    LogSystem.LogWarning("On3DPress--->False", currentTouch.pressure);
                    Notify(currentTouch.pressed, "On3DPress", false);
                    currentTouch.enter = false;
                }
                
            }
            else
            {
                if (currentTouch.pressure > currentTouch.maximumPossiblePressure * 0.5f)
                {
                    if (!currentTouch.enter)
                    {
                        LogSystem.LogWarning("On3DPress--->True", currentTouch.pressure);
                        Notify(currentTouch.pressed, "On3DPress", true);
                        currentTouch.enter = true;
                    }
                }
            }
        }
