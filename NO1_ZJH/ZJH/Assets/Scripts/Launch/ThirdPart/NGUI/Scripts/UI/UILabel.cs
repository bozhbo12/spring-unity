...
public class UILabel : UIWidget
{
	...
	[HideInInspector][SerializeField] public int mSpacingX = 0;
    ...
    string mstrLoadedFont = string.Empty;
    [HideInInspector][SerializeField]string mstrFontName = string.Empty;
    public string FontName
    {
        ...
        set
        {
            //LogSystem.LogError("mstrFontName = "+mstrFontName);
            if (mstrFontName != value)
            {
                ///重新加载字体图集
                LoadAtlas(value);
                mstrFontName = value;
                mstrLoadedFont = mstrFontName;
            }
        }
    }
    ...
    public void ChangeFont()
    {
        ...
        if( !string.IsNullOrEmpty(mstrFontName))
        {
            string strFontName = mstrFontName;
            UIFont uiFont = Config.GetFont(strFontName);
            if( uiFont != null )
            {
                bitmapFont = uiFont;
                MarkAsChanged();
            ...
            {
                if( mstrLoadedFont != mstrFontName )
                {
                    mstrLoadedFont = mstrFontName;
                    LoadAtlas(mstrFontName);
                }
            }
        }
    }
    ...
    public UIFont bitmapFont
    {
        ...
        set
        {
            if (mFont != value)
            {
                ...
                if (!string.IsNullOrEmpty(mstrLoadedFont))
                {
                    if (mFont != null)
                    {
                        CacheObjects.PopCache(mFont.gameObject);
                    }
                    mstrLoadedFont = string.Empty;
                }
    ...
    protected override void OnInit()
    {
        ...
        if (!string.IsNullOrEmpty(mstrFontName) )
        {
            ...
            if (strFontName.Equals("SnailFont"))
            {
                strFontName = "NGUIFont";
            }
            else if (strFontName.Equals("TextFont"))
            {
                strFontName = "NGUIFont1";
            }
            UIFont uiFont = Config.GetFont(strFontName);
            if ( uiFont != null )
            {
    ...
    public override void CheckLoadAtlas()
    {
        ...
        if (bitmapFont == null && !string.IsNullOrEmpty(FontName))
        {
            string strFontName = FontName;
            UIFont uiFont = Config.GetFont(strFontName);
            if (uiFont != null)
            ...
            {
                if (mstrLoadedFont != FontName)
                {
                    mstrLoadedFont = FontName;
                    LoadAtlas(FontName);
                }
            }
        }
    ...
    public override bool CheckWaitLoadingAtlas()
    {
        ...
        return !string.IsNullOrEmpty(FontName) && mFont == null;
    }

    static private string strFontPrefix = "Prefabs/UIAtlas/";
    static private string strFontPrefix2 = "Fonts/";
    static private string strFontSplit = "/";
    private void LoadAtlas(string strFontName)
    {
        if (Application.isPlaying)
        {
            //if (strFontName == "AllNumFont")
            //{
            //    LogSystem.LogWarning("AllNumFont Font Name = " + this.gameObject.name);
            //}

            string strFileName = strFontPrefix + strFontSplit + strFontName;
            if (strFontName.Contains("NGUIFont"))
            {
                strFileName = strFontPrefix2 +  strFontSplit + strFontName;
            }
            //if (strFontName == "AllNumFont")
            //{
            //    LogSystem.LogWarning("AllNumFont Font Name  1= " + strFileName);
            //}

			if(monLoadAsset != null && strFontName.Contains("NGUIFont") )
			{
            ...
            else
            {
                //if (strFontName == "AllNumFont")
                //{
                //    LogSystem.LogWarning("AllNumFont Font Name 2 = " + strFileName);
                //}
                ///编辑器模式
                UnityEngine.Object oAsset = Resources.Load(strFileName);
             //   CacheObjects.PushCache(strFileName, oAsset);
                OnFileLoaded(oAsset, strFileName, null);
            }
        ...
        {
            string strFileName = "Assets/Resources/Prefabs/UIAtlas/" +"/" + strFontName + ".prefab";
            if (strFontName.Contains("NGUIFont"))
            {
                strFileName = "Assets/Resources/Fonts/" +"/" + strFontName + ".prefab";
            }
    ...
    private void OnFileLoaded(UnityEngine.Object oAsset2, string strFileName, VarStore varStore)
    {
        ...
        if (oAsset == null)
        {
            ...
            if (!string.IsNullOrEmpty(mstrFontName))
            {
                LogSystem.LogWarning("加载字体失败---- 字体名字" + mstrFontName + "   UILabel名字" + this.name);
            }
#endif
            //if (monLoaded != null)
            //{
            //    monLoaded(mspList, monAllLoaded, moGo, margs);
            //    mspList = null;
            //    moGo = null;
            //    monLoaded = null;
            //    monAllLoaded = null;
        ...
        UIFont uifont = oAsset.GetComponent<UIFont>();
        if (uifont != null)
        {
            if (!string.IsNullOrEmpty(mstrLoadedFont))
            {
                if (bitmapFont != null)
                    CacheObjects.PopCache(bitmapFont.gameObject);
                mstrLoadedFont = string.Empty;
            }
            bitmapFont = uifont;
            MarkAsChanged();
        ...
        if (mActiveTTF != fnt)
        {
            if (mActiveTTF != null)
            {
                ...
                if (mFontUsage.TryGetValue(mActiveTTF, out usage))
                {
                    ...
                    if (usage == 0)
                    {
#if UNITY_4_6_9 || UNITY_4_7 || UNITY_5
    ...
    protected override void OnAnchor()
    {
        ...
        {
            if (topAnchor.hasTarget && bottomAnchor.hasTarget)
                mOverflow = Overflow.ShrinkContent;
        }
        base.OnAnchor();
    }
    ...
    public static string ConvertNumber(long value)
    {
        ...
        if (value >= hundredmillion)
        {
            ...
            return StringBuilder(tmp, GetUIString("UI230114"));
        }
        else if (value >= tenthousand)
        {
            float tmp = (float)((int)((float)value / 1000)) / 10;
            return StringBuilder(tmp, GetUIString("UI230113"));
        }
    ...
    public int GetCharacterIndex(int currentIndex, KeyCode key)
    {
        if (isValid)
        {
            ...
            if (mTempVerts.Count > 0)
            {
                ...
                for (int i = 0; i < mTempIndices.Count; ++i)
                {
    ...
    public void PrintOverlay(int start, int end, UIGeometry caret, UIGeometry highlight, Color caretColor, Color highlightColor)
    {
        ...
        if (highlight != null && start != end)
        {
            ...
            if (highlight.verts.Count > startingVertices)
            {
                ...
                for (int i = startingVertices; i < highlight.verts.Count; ++i)
                {
        ...
        for (int i = startingCaretVerts; i < caret.verts.Count; ++i)
        {
    ...
    protected Vector2 ApplyOffset(List<Vector3> verts, int start)
    {
    ...
    void ApplyShadow(List<Vector3> verts, List<Vector2> uvs, List<Color32> cols, int start, int end, float x, float y)
    {
    ...
    public void SetCustomScreenCount()
    {
        ...
        {
            float mValue = UIProgressBar.current.value;
            text = SystemSetting.GetScreenCountByRate(mValue).ToString();
        }
    }
    ...
    public void UpdateNGUIText()
    {
        ...
        float fontScale = mScale;
        if (!isDynamic)
        {
            if( mFont != null )
            {
                fontScale =  ((float)mFontSize / mFont.defaultSize) * mScale;
            }
        }
