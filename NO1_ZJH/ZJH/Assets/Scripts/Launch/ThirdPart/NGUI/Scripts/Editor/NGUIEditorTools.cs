...
public class NGUIEditorTools
{
    ...
    static public Rect DrawBackground(Texture2D tex, float ratio)
    {
        ...
        rect.width = ResolutionConstrain.Instance.width - rect.xMin;
    ...
    static public void DrawSeparator()
    {
        ...
        if (Event.current.type == EventType.Repaint)
        {
            ...
            GUI.DrawTexture(new Rect(0f, rect.yMin + 6f, ResolutionConstrain.Instance.width, 4f), tex);
            GUI.DrawTexture(new Rect(0f, rect.yMin + 6f, ResolutionConstrain.Instance.width, 1f), tex);
            GUI.DrawTexture(new Rect(0f, rect.yMin + 9f, ResolutionConstrain.Instance.width, 1f), tex);
            GUI.color = Color.white;
        }
    }
 ...
 || settings.alphaIsTransparency
    ...
    static public void DrawSprite(Texture2D tex, Rect drawRect, UISpriteData sprite, Color color, Material mat)
    {
        ...
        EditorGUI.DropShadowLabel(GUILayoutUtility.GetRect(ResolutionConstrain.Instance.width, 18f), text);
    }
    ...
    static public void DrawSpriteField2(string label, SerializedObject ob, SerializedProperty atlas, SerializedProperty sprite, params GUILayoutOption[] options)
    {
        DrawSpriteField2(label, ob, atlas, sprite, 76f, false, false, options);
    }
    ...
    /// <param name="options"></param>
    static public void DrawSpriteField2(string label, SerializedObject ob, SerializedProperty atlas, SerializedProperty sprite, bool removable, params GUILayoutOption[] options)
    {
        DrawSpriteField2(label, ob, atlas, sprite, 76f, false, removable, options);
    }
    /// <summary>
    /// by zhangrj UIsprite中去除了Atlas
    /// </summary>
    ...
    /// <param name="removable"></param>
    /// <param name="options"></param>
    static public void DrawSpriteField2(string label, SerializedObject ob, SerializedProperty atlas, SerializedProperty sprite, float width, bool padded, bool removable, params GUILayoutOption[] options)
    {
        if (atlas != null && !string.IsNullOrEmpty( atlas.stringValue ))
        {
            GUILayout.BeginHorizontal();
            ...
            else
            {
                ...
                {
                    if (GUILayout.Button(spriteName, "MiniPullDown", options))
                        SpriteSelector.Show(ob, sprite, atlas.stringValue);
                }
                EditorGUI.EndDisabledGroup();

                EditorGUI.BeginDisabledGroup(!removable);
#if UNITY_3_5
				if (GUILayout.Button("X", GUILayout.Width(20f))) sprite.stringValue = string.Empty;
#else
                if (GUILayout.Button(string.Empty, "ToggleMixed", GUILayout.Width(20f))) sprite.stringValue = string.Empty;
#endif
                EditorGUI.EndDisabledGroup();
    ...
    static public int GetClassID(System.Type type)
    {
        ...
        NGUITools.DestroyImmediateNGUI(go);
        return classID;
    }
