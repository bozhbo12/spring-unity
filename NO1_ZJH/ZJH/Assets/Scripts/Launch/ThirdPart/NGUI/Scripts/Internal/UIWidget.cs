...
public class UIWidget : UIRect
{
    ...
    private bool mbHasPanel = false;
    public UIPanel panel
    {
        set
        {
            uiPanel = value;
            mbHasPanel = value != null;
        }
        get
        {
            return uiPanel;
    ...
    /// 是否需要初始化
    /// </summary>
    protected bool colliderInit = false;
    ...
    /// <summary>
    /// 加载完成回调
    /// 原先是直接Atlas或者UIFont直接保存的，现在是保存name然后load进来的
    /// 所以外部调用的时候Atlas和Font一开始的时候会为空
    /// 使用+=，或者-=
    /// </summary>
    ...
    public int width
    {
        ...
        set
        {
            ...
            if (mWidth != value && keepAspectRatio != AspectRatioSource.BasedOnHeight)
            {
                ...
                {
                    if (leftAnchor.hasTarget  && rightAnchor.hasTarget)
                    {
    ...
    public int height
    {
        ...
        set
        {
            ...
            if (mHeight != value && keepAspectRatio != AspectRatioSource.BasedOnWidth)
            {
                ...
                {
                    if (bottomAnchor.hasTarget && topAnchor.hasTarget)
                    {
                    ...
                    else if (bottomAnchor.hasTarget)
                    {
                        NGUIMath.AdjustWidget(this, 0f, 0f, 0f, value - mHeight);
                    }
    ...
    public int depth
    {
        ...
        set
        {
            if (mDepth != value)
            {
                if (mbHasPanel) panel.RemoveWidget(this);
                mDepth = value;
    ...
    public int raycastDepth
    {
        get
        {
            if (!mbHasPanel) CreatePanel();
            return (mbHasPanel) ? mDepth + panel.depth * 1000 : mDepth;
        }
    }
    ...
    public virtual Vector4 drawingDimensions
    {
        get
        {
            ...
            vTemp.x = mDrawRegion.x == 0f ? x0 : Mathf.Lerp(x0, x1, mDrawRegion.x);
            vTemp.y = mDrawRegion.y == 0f ? y0 : Mathf.Lerp(y0, y1, mDrawRegion.y);
            vTemp.z = mDrawRegion.z == 1f ? x1 : Mathf.Lerp(x0, x1, mDrawRegion.z);
            vTemp.w = mDrawRegion.w == 1f ? y1 : Mathf.Lerp(y0, y1, mDrawRegion.w);
            return vTemp;
        }
    }
    ...
    protected void UpdateFinalAlpha(int frameID)
    {
        ...
        {
            finalAlpha = (mbHasParent) ? ParentPanel.CalculateFinalAlpha(frameID) * mColor.a : mColor.a ;
        }
    }
    ...
    public void SetRect(float x, float y, float width, float height)
    {
        ...
        if (isAnchored)
        {
            ...
            if (leftAnchor.hasTarget) leftAnchor.SetHorizontal(t, x);
            if (rightAnchor.hasTarget) rightAnchor.SetHorizontal(t, x + width);
            if (bottomAnchor.hasTarget) bottomAnchor.SetVertical(t, y);
            if (topAnchor.hasTarget) topAnchor.SetVertical(t, y + height);
#if UNITY_EDITOR
            NGUITools.SetDirty(this);
#endif
        }
    ...
    public void ResizeCollider()
    {
        ...
        {
            mCollider = GetComponent<Collider>() as BoxCollider;
            if (mCollider != null) NGUITools.UpdateWidgetCollider(this,mCollider, true);
        }
    }
    ...
    /// Calculate the widget's bounds, optionally making them relative to the specified transform.
    /// </summary>
    static Bounds tempZero = new Bounds(Vector3.zero,Vector3.zero);
    public Bounds CalculateBounds(Transform relativeParent)
    {
        ...
        else
        {
            ...
            Bounds b =tempZero;
            b.center = toLocal.MultiplyPoint3x4(corners[0]);
            b.size = Vector3.zero;
    ...
    public virtual void MarkAsChanged()
    {
        if (NGUITools.GetActive(this))
        {
            ...
            if (mbHasPanel && enabled && NGUITools.GetActive(gameObject) && !mPlayMode)
            {
                SetDirty();
                CheckLayer();
#if UNITY_EDITOR
            ...
            {
                isMakePixelPerfect = false;
                MakePixelPerfect();
            }

            if (onMarkAsChangedEvent != null)
                onMarkAsChangedEvent();
        }
    }
    ...
    {
        if (mStarted && !mbHasPanel && enabled && NGUITools.GetActive(gameObject))
        {
    ...
    {
        if (mbHasPanel && panel.gameObject.layer != gameObject.layer)
        {
    ...
    protected override void OnInit()
    {
        ...
        if (mCollider != null && autoResizeBoxCollider)
            colliderInit = true;
        // Prior to NGUI 2.7.0 width and height was specified as transform's local scale
    ...
    protected override void OnAnchor()
    {
        ...
        if (leftAnchor.tranTarget == bottomAnchor.tranTarget &&
            leftAnchor.tranTarget == rightAnchor.tranTarget &&
            leftAnchor.tranTarget == topAnchor.tranTarget)
        {
        ...
        newPos.x =Mathf.Lerp(lt, rt, pvt.x) ;
        newPos.y = Mathf.Lerp(bt, tt, pvt.y);
        newPos.z =pos.z ;
    ...
    protected override void OnUpdate()
    {
        ...
        if (colliderInit)
        {
            if (mCollider != null)
            {
                float w = mCollider.size.x;
                float h = mCollider.size.y;
                if (w < 0.01f && h < 0.01f)
                {
                    ResizeCollider();
                }
    ...
    void AllocateGeometry()
    {
        if (geometry == null)
        {
            if (this is UILabel)
            {
                UILabel lable = this as UILabel;
                if( lable != null && lable.effectStyle != UILabel.Effect.None)
                {
                    geometry = UIGeometry.CreateStyleLabelGeometry();
                }
                else
                {
                    geometry = UIGeometry.CreateLabelGeometry();
                }
            }
            else
            {
                geometry = UIGeometry.CreateSpriteGeometry();
            }
        }
    }
    void CollectGeometry()
    {
        if( geometry != null )
        {
            if (this is UILabel)
            {
                UIGeometry.CollectLabelGeometry(geometry);
            }
            else
            {
                UIGeometry.CollectSpriteGeometry(geometry);
            }
            geometry = null;
        }
    }
    ...
    private string strUICamera= "UICamera";
    public Transform GetWidgetRoot()
    {
        ...
        while ( trans != null )
        {
            ...
            {
                if( trans.parent.CompareTag(strUICamera) )
                {
                    return trans;
                }
            }
    ...
    public void OnWidgetDestroy()
    {
        if (Application.isPlaying)
        {
            ...
            {
                UILabel lbl = this as UILabel;
                if (lbl != null && lbl.bitmapFont != null)
                {

                    if (!string.IsNullOrEmpty(lbl.FontName))
                    {
                        //CacheObjects.PopCachePrefab(lbl.bitmapFont.gameObject);
                        lbl.bitmapFont = null;
                    }
            ...
            if (this is UISprite)
            {
                UISprite sp = this as UISprite;
                if (sp != null && sp.atlas != null)
                {
                    if (!string.IsNullOrEmpty(sp.AtlasName))
                    {
                        //CacheObjects.PopCachePrefab(sp.atlas.gameObject);
                        sp.atlas = null;
                    }
                }
            }
    ...
    void OnDrawGizmos()
    {
        if (isVisible && NGUITools.GetActive(this))
        {
            ...
            center.x = mWidth * (0.5f - offset.x);
            center.y = mHeight * (0.5f - offset.y);
            center.z = -mDepth * adjustment;
    ...
    {
        return geometry.GetRtpVertsCount();
    }
    /// <summary>
    ...
    virtual public void MakePixelPerfect()
    {
        ...
        Vector3 vTemp = Vector3.zero ;
        vTemp.x = Mathf.Sign(ls.x);
        vTemp.y = Mathf.Sign(ls.y);
        vTemp.z = 1f;
        cachedTransform.localScale = vTemp;
    }
    ...
    virtual public void OnFill(List<Vector3> verts, List<Vector2> uvs, List<Color32> cols) { }
}
